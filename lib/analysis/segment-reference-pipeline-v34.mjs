...
export async function runSegmentReferencePipelineV34({ record, providerCall }) {
  ...
  for (const [name, fields] of passSpecs) {
    ...
    try {
      ...
      const validation = validateCompactPass(parsed, complexity.segments, fields);
      passes.push({ name, status: validation.valid ? 'completed' : 'validation_failed', latency_ms: response.latency_ms ?? Date.now() - started, usage: response.usage ?? null, request_id: response.request_id ?? null, parsed_claims: parsed, validation: { valid: validation.valid, errors: validation.errors }, regrouped: validation.regrouped });
    } catch (error) { passes.push({ name, status: 'failed', error: String(error?.code ?? 'provider_error'), latency_ms: Date.now() - started, usage: null }); }
  }
