...
export async function executeRoutedAnalysisJob({
  ...
  if (first.run) {
    first.run.estimated_cost_micros = actualCost(economy, first.run);
    budget.recordActual({
      estimatedCostMicros: economyEstimate,
      actualCostMicros: first.run.estimated_cost_micros,
    });
  ...
  if (reasons.length) {
    ...
    if (auth.allowed) {
      ...
      if (second.run) {
        second.run.estimated_cost_micros = actualCost(escalation, second.run);
        budget.recordActual({
          estimatedCostMicros: escalationEstimate,
          actualCostMicros: second.run.estimated_cost_micros,
        });
