 ...
 * }} input
 */
export function validateSemanticReviewDecision({
  decision,
  correctedOutput = null,
  existingOutput = null,
}) {
  if (!SEMANTIC_REVIEW_DECISIONS.includes(decision)) {
    return { valid: false, errors: ["invalid_review_decision"], effective_output: null };
  }
  const effectiveOutput = correctedOutput ?? existingOutput;
  if (decision !== "approve") {
    return { valid: true, errors: [], effective_output: effectiveOutput };
  }
  const schema = validateAnalysisSchema(effectiveOutput);
  return {
    valid: schema.valid,
    errors: schema.valid ? [] : ["approve_effective_output_schema_invalid"],
...
export function buildSemanticReviewEvent({
  ...
  const validation = validateSemanticReviewDecision({
    decision,
    correctedOutput,
    existingOutput: result.structured_result,
  });
  if (!validation.valid) throw new Error(validation.errors[0]);
  const effectiveOutput = validation.effective_output;
  return {
    id: randomUUID(),
    result_id: result.id,
