...
export function reconcileAnalysisJobs(input = {}) {
  ...
  const completed = matchingJobs.find((job) => clean(job.status) === "succeeded")
    || existingResults.find((result) =>
      clean(result.revision_id) === revisionId
      && clean(result.analysis_schema_version ?? result.schema_version) === schemaVersion
      && clean(result.input_fingerprint) === inputFingerprint
      && clean(result.result_status) === "validated");

  if (completed) {
    return {
      action: "no_action_completed",
      job: matchingJobs.find((job) => clean(job.status) === "succeeded") ?? null,
      readiness,
