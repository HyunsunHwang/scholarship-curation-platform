...
import { validateAnalysisSchema, validateStructuredAnalysis } from "./analysis-validator.mjs";
...
export function evaluateAnalysisCandidates(rows, candidates, { candidateName = "replay" } = {}) {
  ...
  for (const gold of rows) {
    ...
    const analysisInput = candidate.analysis_input ?? null;
    if (validateAnalysisSchema(output).valid) totals.schema_valid += 1;
    if (output?.evidence?.length) totals.evidence_present += 1;
    const evidenceValidation = analysisInput
      ? validateStructuredAnalysis(output, { analysisInput })
      : { valid: Boolean(output?.evidence?.length), errors: [] };
    if (evidenceValidation.valid) totals.evidence_valid += 1;
    else totals.evidence_invalid += 1;
    ...
    totals.actual_cost_micros += Number(candidate.actual_cost_micros ?? candidate.estimated_cost_micros ?? 0);
    totals.latency_ms += Number(candidate.latency_ms ?? 0);
    totals.escalated += candidate.escalated ? 1 : 0;
  }
  const evidence_validation_rate = totals.rows
    ? totals.evidence_valid / totals.rows
    : 0;
  return {
    evaluation_version: "analysis-evaluation-v2",
