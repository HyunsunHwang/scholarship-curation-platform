...
import {
  buildPrivacyRedactionContract,
  isPrivacyAllowedForProviderCall,
} from "./analysis-privacy-contract.mjs";
...
function hasUsableKoreanText(text, { minLength = 80 } = {}) {
  const value = clean(text);
  if (value.length < minLength) return false;
  if (replacementCharRatio(value) >= 0.03) return false;
...
function privacyRiskSignal(text) {
  ...
  if (/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.test(value) && /(명단|합격|수혜)/u.test(value)) {
    return true;
  }
...
function collectAttachmentTexts(assets = []) {
  return (Array.isArray(assets) ? assets : [])
    .map((asset) => clean(asset.extracted_text ?? asset.extractedText ?? asset.metadata?.extracted_text))
    .filter(Boolean);
}
...
function assetExtractionState(assets = []) {
  ...
  if (rows.length === 0) {
    return {
      ...
      usable_attachment_text: false,
      attachment_text_quality: "none",
      attachment_text: "",
    };
  }
  let pending = 0;
  ...
  const attachmentText = texts.join("\n");
  const usable = hasUsableKoreanText(attachmentText, { minLength: 80 });
  const poor = attachmentText.length > 0
    && (replacementCharRatio(attachmentText) >= 0.03 || attachmentText.length < 40);
  return {
    has_attachments: true,
    extraction_pending: pending > 0 && extracted < rows.length,
    extracted_text_count: extracted,
    usable_attachment_text: usable,
    attachment_text_quality: usable ? "good" : (poor ? "poor" : (attachmentText ? "insufficient" : "none")),
...
export function evaluateAnalysisReadiness(input = {}) {
  ...
  const privacyContract = buildPrivacyRedactionContract({
    status: input.privacy_status ?? input.privacy?.status ?? "not_scanned",
  ...
  if (!revision || !clean(revision.id)) {
    return {
      ...
      input_profile: {
        ...
        privacy_status: privacyContract.status,
      },
    };
  }
  ...
  const attachmentTexts = collectAttachmentTexts(assets);
  const hasBodyText = hasUsableKoreanText(body, { minLength: 80 });
  const hasAttachmentText = assetState.usable_attachment_text;
  const bodyLowQuality = replacementCharRatio(combined) >= 0.03
    || (body.length > 0 && body.length < 40);
  const attachmentLowQuality = assetState.attachment_text_quality === "poor";
  const lowQuality = bodyLowQuality || attachmentLowQuality;
  const rosterOrResult = looksLikeRosterOrResult(`${combined}\n${assetState.attachment_text}`)
  ...
  const privacyRisk = Boolean(input.personal_data_risk)
    || privacyRiskSignal(`${combined}\n${assetState.attachment_text}`)
    || privacyContract.status === "blocked";
  ...
  const candidateConfirmed = candidateClassification === "candidate" || scholarshipDedicated;

  const usableTextQuality = hasBodyText || hasAttachmentText
    ? "good"
    : (lowQuality ? "poor" : "insufficient");

  const inputProfile = {
    has_body_text: hasBodyText,
    has_attachment_text: hasAttachmentText,
    korean_text_quality: usableTextQuality,
    ...
    attachment_text_quality: assetState.attachment_text_quality,
    privacy_status: privacyContract.status,
  };

  if (["not_candidate", "out_of_range"].includes(candidateClassification) && !scholarshipDedicated) {
    return {
      ready: false,
  ...
  if (!candidateConfirmed) {
    return {
      ready: false,
      status: "excluded_not_candidate",
      reason_codes: ["CANDIDATE_NOT_CONFIRMED"],
      input_profile: inputProfile,
    };
  }
  ...
  if (noticeType !== "new_recruitment") {
    return {
      ready: false,
      status: "excluded_notice_type",
      reason_codes: ["NOTICE_TYPE_NOT_NEW_RECRUITMENT"],
      input_profile: inputProfile,
    };
  }

  if (!isPrivacyAllowedForProviderCall(privacyContract.status)) {
    return {
      ready: false,
      status: "excluded_privacy_risk",
      reason_codes: privacyContract.status === "not_scanned"
        ? ["PRIVACY_NOT_SCANNED"]
        : ["PERSONAL_DATA_RISK"],
      input_profile: inputProfile,
    };
  ...
  if (attachmentTexts.length > 0 && attachmentLowQuality && !hasBodyText) {
    return {
      ready: false,
      status: "excluded_low_text_quality",
      reason_codes: ["LOW_ATTACHMENT_TEXT_QUALITY"],
      input_profile: inputProfile,
    };
  }

  if (lowQuality && !hasAttachmentText) {
    return {
      ready: false,
      status: "excluded_low_text_quality",
