...
import {
  ...
  normalizeProgramOrganization as scholarshipNormalizeOrganization,
} from "./scholarship-identity.mjs";
...
export {
  buildCycleIdentity,
  buildProgramIdentity,
  scholarshipIdentityNormalize as normalizeProgramName,
  scholarshipNormalizeOrganization as normalizeProgramOrganization,
};

function fingerprint(value) {
  return analysisSha256(JSON.stringify(value));
}
...
export function findExistingProgramCandidates({
  ...
  const normalizedName = scholarshipIdentityNormalize(programName);
  const normalizedOrganization = scholarshipNormalizeOrganization(operatingOrganization);
  ...
  for (const program of programs) {
    const nameMatch = scholarshipIdentityNormalize(program.canonical_name) === normalizedName
      || aliasProgramIds.has(program.id);
    if (!nameMatch) continue;
    const organizationMatch = scholarshipIdentityNormalize(program.operating_organization)
...
export function buildProgramCycleProposal({
  ...
  const proposedProgram = {
    ...
    normalized_name: scholarshipIdentityNormalize(programName),
...
export function approveProgramCycleProposal({
  ...
  if (decision === "approve_new_program_and_cycle") {
    const canonicalName = programPatch.canonical_name ?? proposal.proposed_program.canonical_name;
    const operatingOrganization = programPatch.operating_organization
      ?? proposal.proposed_program.operating_organization;
    const identity = buildProgramIdentity({
      canonicalName,
      operatingOrganization,
      discriminator: programPatch.identity_discriminator
        ?? proposal.proposed_program.identity_discriminator,
    });
    if (next.programs.some((row) => (
      row.identity_key ?? buildProgramIdentity({
        canonicalName: row.canonical_name,
        operatingOrganization: row.operating_organization,
        discriminator: row.identity_discriminator,
      }).identity_key
    ) === identity.identity_key)) {
      return { ok: false, reason: "program_identity_conflict", state: cloneState(state) };
    }
    const programId = stableAnalysisUuid("scholarship_programs", identity.identity_key);
    program = {
      id: programId,
  ...
  if (decision === "approve_existing_program_existing_cycle") {
    ...
    const identity = buildCycleIdentity({
      programId: program.id,
      revisionId: proposal.revision_id,
      cycleYear: cyclePatch.cycle_year ?? proposal.proposed_cycle.cycle_year,
      academicTerm: cyclePatch.academic_term ?? proposal.proposed_cycle.academic_term,
      applicationStartAt: cyclePatch.application_start_at
        ?? proposal.proposed_cycle.application_start_at,
      applicationEndAt: cyclePatch.application_end_at
        ?? proposal.proposed_cycle.application_end_at,
    });
    const conflict = next.cycles.find((row) =>
      row.revision_identity_key === identity.revision_identity_key
      || (identity.cycle_identity_key && row.cycle_identity_key === identity.cycle_identity_key));
    if (conflict) {
      return { ok: false, reason: "cycle_identity_conflict", state: cloneState(state) };
    }
    const ambiguous = next.cycles.find((row) =>
      row.program_id === program.id
      && Number(row.cycle_year) === Number(
        cyclePatch.cycle_year ?? proposal.proposed_cycle.cycle_year,
      )
      && clean(row.cycle_label) === clean(
        cyclePatch.cycle_label ?? proposal.proposed_cycle.cycle_label,
      ));
    if (!identity.cycle_identity_key && ambiguous) {
      return { ok: false, reason: "cycle_identity_review_required", state: cloneState(state) };
    }
    const logicalKey = identity.revision_identity_key;
