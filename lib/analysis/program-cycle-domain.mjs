...
export function normalizeProgramOrganization(value) {
  return normalizeProgramName(value) || "organization-unknown";
}
...
export function buildProgramIdentity({
  canonicalName,
  operatingOrganization,
  discriminator = "default",
}) {
  const normalizedName = normalizeProgramName(canonicalName);
  const normalizedOrganization = normalizeProgramOrganization(operatingOrganization);
  const normalizedDiscriminator = normalizeProgramName(discriminator) || "default";
  return {
    normalized_name: normalizedName,
    normalized_organization: normalizedOrganization,
    identity_discriminator: normalizedDiscriminator,
...
export function buildCycleIdentity({
  ...
  const revisionIdentityKey = fingerprint({ program_id: programId, revision_id: revisionId });
  const term = normalizeProgramName(academicTerm);
  const hasClearSemanticIdentity = Boolean(
    cycleYear && (term || (applicationStartAt && applicationEndAt)),
  );
  return {
    revision_identity_key: revisionIdentityKey,
    cycle_identity_key: hasClearSemanticIdentity
      ? fingerprint({
        program_id: programId,
        cycle_year: Number(cycleYear),
        academic_term: term || null,
        application_start_at: applicationStartAt || null,
        application_end_at: applicationEndAt || null,
      })
      : null,
  };
}
