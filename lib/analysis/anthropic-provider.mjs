...
function sanitizeProviderErrorMessage(value) {
  const compact = String(value ?? "").replace(/\s+/gu, " ").trim()
    .replace(/(?:sk-[A-Za-z0-9_-]{12,}|(?:bearer|api[_ -]?key)\s+[A-Za-z0-9._-]{12,})/giu, "[REDACTED]")
    .replace(/(prompt|body_text|segments)\s*[:=].*$/iu, "$1: [REDACTED]");
  return compact.slice(0, 500) || null;
}
  ...
  try {
    ...
    if (!response.ok) {
      throw Object.assign(new Error(`Anthropic request failed (${response.status})`), {
        ...
        provider_error_type: typeof payload?.error?.type === "string" ? payload.error.type : typeof payload?.type === "string" ? payload.type : null,
        provider_error_code: typeof payload?.error?.code === "string" ? payload.error.code : null,
        provider_error_message: sanitizeProviderErrorMessage(payload?.error?.message ?? payload?.message),
    ...
    if (error?.name === "AbortError" && controller.signal.aborted) {
      throw Object.assign(new Error("Anthropic request timed out"), { code: "provider_timeout", timeout_ms: timeoutMs });
    }
    if (error?.code?.startsWith("provider_")) throw error;
    throw Object.assign(new Error("Anthropic network request failed"), {
      code: "provider_network_error",
      safe_error_name: String(error?.name ?? "Error"),
      safe_error_code: typeof error?.code === "string" ? error.code : null,
    });
  } finally {
    clearTimeout(timer);
  }
