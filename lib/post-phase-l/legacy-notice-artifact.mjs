...
export function normalizeLegacyNoticeRow(row = {}) {
  ...
  return {
    ...
    diagnostic_observation: Boolean(
      row.diagnostic_observation ?? row.diagnosticObservation,
    ),
    downstream_eligibility: clean(row.downstream_eligibility ?? row.downstreamEligibility) || null,
...
export function parseLegacyNoticeCsv(text) {
  ...
  for (const cells of body) {
    ...
    const row = normalizeLegacyNoticeRow({
    ...
    if (seenUrls.has(noticeUrl)) {
      duplicateUrlCount += 1;
      droppedRecords.push({ ...row, drop_reason: "legacy_global_url_dedupe" });
      continue;
    }
...
export function applyLegacyNoticeUrlDedupe(records = []) {
  ...
  for (const row of records) {
    ...
    if (seenUrls.has(noticeUrl)) {
      ...
      dropped.push({
        ...row,
        drop_reason: "legacy_global_url_dedupe",
      });
      continue;
    }
    seenUrls.add(noticeUrl);
...
export function loadLegacyNoticeArtifact(input, options = {}) {
  ...
  if (Array.isArray(input)) {
    ...
    const deduped = applyDedupe
      ...
      : { records: normalized, duplicate_url_count: 0, dropped_records: [] };
    return {
      records: deduped.records,
      duplicate_url_count: deduped.duplicate_url_count,
      dropped_records: deduped.dropped_records ?? [],
  ...
  if (typeof input === "string") {
    ...
    const parsed = parseLegacyNoticeCsv(text);
    return { ...parsed, dropped_records: parsed.dropped_records ?? [], source: resolved };
  }
