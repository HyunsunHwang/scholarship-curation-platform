...
function mergeNoticeGroup(notices) {
  ...
  const seenAppLinks = new Set();
  let candidateClassification = null;
  let diagnosticObservation = false;
  let downstreamEligibility = null;
  for (const notice of notices) {
    const links = notice.application_links ?? notice.applicationLinks ?? [];
    if (Array.isArray(links)) {
      for (const link of links) {
        const url = clean(typeof link === "string" ? link : link?.url);
        if (!url || seenAppLinks.has(url)) continue;
        seenAppLinks.add(url);
        applicationLinks.push(typeof link === "string" ? { url: link } : link);
      }
    }
    candidateClassification = candidateClassification
      || clean(notice.candidate_classification ?? notice.candidateClassification)
      || null;
    diagnosticObservation = diagnosticObservation
      || Boolean(notice.diagnostic_observation ?? notice.diagnosticObservation);
    downstreamEligibility = downstreamEligibility
      || clean(notice.downstream_eligibility ?? notice.downstreamEligibility)
      || null;
  }
  ...
  return {
    ...
    normalizedPayload: {
      ...
      ...(inlineSections.length > 0
        ...
        : {}),
      ...(applicationLinks.length > 0 ? { application_links: applicationLinks } : {}),
      ...(candidateClassification
        ? { candidate_classification: candidateClassification }
        : {}),
      ...(diagnosticObservation ? { diagnostic_observation: true } : {}),
      ...(downstreamEligibility ? { downstream_eligibility: downstreamEligibility } : {}),
    },
