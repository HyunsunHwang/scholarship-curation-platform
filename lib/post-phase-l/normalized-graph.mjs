  ...
  for (const result of sourceResults) {
    ...
    for (const notice of notices) {
      ...
      const identityKey = inlineSectionId
        ...
        : externalArticleId ? `external:${externalArticleId}` : `url:${canonicalUrlHash}`;
      ...
      plan.tables.ingestion_notices.push({
        id: noticeId,
        source_id: sourceId,
        identity_kind: inlineSectionId ? "inline_section_id" : externalArticleId ? "external_article_id" : "canonical_detail_url",
      ...
      for (const alias of aliases) {
        ...
        plan.tables.ingestion_notice_url_aliases.push({
          id: stableUuid("ingestion_notice_url_aliases", `${noticeId}|${alias.kind}|${normalizedHash}`),
          notice_id: noticeId,
          source_id: sourceId,
      ...
      plan.tables.crawled_notices_compatibility.push({
        ...
        title,
        // Legacy compatibility uses its URL as a uniqueness key. Inline notices
        // intentionally share one canonical page, so retain their display fragment here.
        notice_url: inlineSectionId ? (originalUrl || canonicalUrl) : canonicalUrl,
