...
function inlineDateEvidence(section) {
  ...
  const evidence = [];
  const normalizedValue = (date) => {
    const parts = date.match(/(20\d{2})\D+(\d{1,2})\D+(\d{1,2})/);
    return parts ? `${parts[1]}-${parts[2].padStart(2, "0")}-${parts[3].padStart(2, "0")}` : null;
  };
  const append = (rawText, role, evidenceText, confidence) => {
    const normalized = normalizedValue(rawText);
    const key = `${normalized ?? rawText}|${role}`;
    if (evidence.some((entry) => `${entry.normalizedValue ?? entry.value}|${entry.role}` === key)) return;
    evidence.push({ rawText, normalizedValue: normalized, value: rawText, role, evidenceText, evidence: evidenceText, confidence });
  };
  for (const value of section.publishedDateTexts ?? []) {
    for (const date of dateValues(value)) append(date, "published_date", clean(value), "explicit_selector");
  }
  ...
  for (const [index, date] of dates.entries()) {
    ...
    const role = /신청|접수|application/.test(context) ? (index > 0 && /[~～\-]|부터|까지/.test(text) ? "application_end_date" : "application_start_date")
    ...
    append(date, role, context, role === "unknown_date" ? "unclassified" : "explicit_context");
  }
  return evidence;
}

function stableSectionIdentity(sourceId, section, duplicateTitle, usedIdentities, dateEvidence, links) {
  ...
  if (anchor) {
    identity = `${identity}-${createHash("sha256").update(anchor).digest("hex").slice(0, 12)}`;
    usedIdentities.add(identity);
    return { id: identity, stability: "strong", basis: "heading_anchor" };
  }
  const published = dateEvidence.find((entry) => entry.role === "published_date")?.normalizedValue;
  if (duplicateTitle && published) {
    identity = `${identity}-${createHash("sha256").update(published).digest("hex").slice(0, 12)}`;
    usedIdentities.add(identity);
    return { id: identity, stability: "strong", basis: "title_published_date" };
  }
  const authoritativeLink = links.find((link) => ["application_form", "cross_origin_authoritative_detail"].includes(link.role))?.url;
  if (duplicateTitle && authoritativeLink) {
    identity = `${identity}-${createHash("sha256").update(authoritativeLink).digest("hex").slice(0, 12)}`;
    usedIdentities.add(identity);
    return { id: identity, stability: "strong", basis: "title_authoritative_link" };
  }
  if (duplicateTitle || usedIdentities.has(identity)) {
    ...
    usedIdentities.add(identity);
    return { id: identity, stability: "weak", basis: "duplicate_title_fallback_fingerprint" };
  }
  usedIdentities.add(identity);
  return { id: identity, stability: "strong", basis: "unique_title" };
}
...
export function extractInlineSectionNotices(source = {}, html = "") {
  ...
  const notices = extractableSections
    .map((section) => {
      ...
      const identity = stableSectionIdentity(
        source.sourceId,
        section,
      ...
      const inlineSectionId = identity.id;
      ...
      const publishedDate = dateEvidence.find((entry) => entry.role === "published_date")?.rawText ?? "";
      return {
