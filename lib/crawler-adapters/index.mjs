...
export async function fetchCauPortalList(source, options = {}) {
  ...
  for (let pageNo = 1; pageNo <= maxPages; pageNo += 1) {
    ...
    const listRes = await transportClient.fetchJson(apiUrl, {
      ...
      retryCount: 0,
      signal,
      // This CMS returns its JSON payload as text/html, and returns an HTML
      // document when explicitly asked for application/json. Keep the request
      // declarative at the adapter boundary while validating the JSON shape
      // below.
      accept: "*/*",
      method: "POST",
      headers: {
    ...
    if (!payload || typeof payload !== "object" || !payload.data || !Array.isArray(payload.data.list)) {
      const error = new Error("Board API JSON response omitted data.list.");
