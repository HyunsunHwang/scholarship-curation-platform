...
function canonicalHostnames(values = []) {
...
export function resolveEffectiveTransportPolicy({
  ...
  for (const binding of applicable) {
    ...
    if (binding.allowedHttpHosts) allowedHttpHosts = [...binding.allowedHttpHosts];
  ...
  allowedHosts = canonicalHostnames(allowedHosts);
  allowedHttpHosts = canonicalHostnames(allowedHttpHosts);
  let sourceProtocol = "";
  try { sourceProtocol = new URL(source.listUrl).protocol; } catch {}
  if (policy.protocolMode === "preserve-http" && sourceProtocol === "http:") {
    allowedHttpHosts = canonicalHostnames([...allowedHttpHosts, sourceHostname]);
  }
  ...
  if (policy.tlsMode === "system-ca") {
    if (!allowedHosts.includes(sourceHostname) || !reason) {
      const error = new Error(`${source.sourceId}: system CA policy is missing an exact host or reason`);
      error.code = "transport_policy_system_ca_invalid";
      throw error;
    }
  }
  if (allowedHttpHosts.length > 0 && policy.protocolMode === "strict") {
    const error = new Error(`${source.sourceId}: strict policy cannot authorize HTTP hosts`);
    error.code = "transport_policy_http_policy_conflict";
    throw error;
  }
  ...
  return Object.freeze({
    ...
    allowedHttpHosts: Object.freeze(allowedHttpHosts),
    userAgentProfile: runtimeOverrides.userAgent
      ? "runtime-override"
      : policy.userAgentProfile,
    userAgent,
