...
function canonicalHostnameSet(values = []) {
  return JSON.stringify([...new Set(values)].sort((left, right) => left.localeCompare(right)));
}

function bindingTrustConflicts(left, right) {
  const conflicts = [];
  for (const field of ["allowedHosts", "allowedHttpHosts"]) {
    if (left[field] && right[field]
      && canonicalHostnameSet(left[field]) !== canonicalHostnameSet(right[field])) {
...
export function validateTransportPolicyRegistry({
  ...
  for (let leftIndex = 0; leftIndex < bindings.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < bindings.length; rightIndex += 1) {
      ...
      if (left.priority !== right.priority || !bindingsCanOverlap(left, right, sources)) continue;
      if (policyConflicts(bindingPolicy(registry, left), bindingPolicy(registry, right))) {
        errors.push(
          `${left.bindingId}/${right.bindingId}: conflicting bindings at priority ${left.priority}`,
        );
      }
      for (const field of bindingTrustConflicts(left, right)) {
        errors.push(
          `${left.bindingId}/${right.bindingId}: conflicting ${field} at priority ${left.priority}`,
        );
      }
    }
  }
