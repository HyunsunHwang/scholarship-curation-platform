...
function exactHost(value) {
  ...
  return hostname && exactHostnamePattern.test(hostname) && !hostname.includes("*")
    ? hostname
    : null;
}
...
export function validateTransportPolicyRegistry({
  ...
  for (const binding of registry.bindings ?? []) {
    ...
    for (const field of ["sourceIds", "universitySlugs", "hosts", "allowedHosts"]) {
      for (const value of binding[field] ?? []) {
        if (value !== clean(value)) errors.push(`${binding.bindingId}.${field}: values must be trimmed`);
      }
    }
    for (const host of [...(binding.hosts ?? []), ...(binding.allowedHosts ?? [])]) {
      if (!exactHost(host)) {
