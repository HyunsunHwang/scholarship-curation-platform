...
const NON_RETRYABLE_CONTENT_CODES = new Set([
  "unexpected_content_type",
  "json_decode_error",
  "json_shape_mismatch",
]);
const JSON_CONTENT_TYPE_POLICIES = new Set(["body-json", "strict"]);
...
function normalizedMediaType(value) {
  return clean(value).split(";", 1)[0].trim().toLowerCase();
}

function normalizeExpectedContentTypes(value) {
  const contentTypes = Array.isArray(value) && value.length > 0
    ? value
    : ["application/json"];
  return contentTypes.map(normalizedMediaType).filter(Boolean);
}

function matchesExpectedContentType(contentType, expectedContentTypes) {
  const actual = normalizedMediaType(contentType);
  return Boolean(actual) && expectedContentTypes.some((expected) => (
    actual === expected
    || (expected === "application/json" && actual.endsWith("+json"))
  ));
}

function isRetryableError(error) {
  if (error?.name === "AbortError" || error?.code === "request_timeout") return true;
  if (NON_RETRYABLE_CONTENT_CODES.has(clean(error?.code))) return false;
...
export function createTransportClient({
  ...
  return Object.freeze({
    ...
    async fetchJson(url, options = {}) {
      const contentTypePolicy = clean(options.contentTypePolicy || "body-json").toLowerCase();
      if (!JSON_CONTENT_TYPE_POLICIES.has(contentTypePolicy)) {
        const error = new TypeError(`Unsupported JSON content type policy: ${contentTypePolicy}`);
        error.code = "invalid_content_type_policy";
        throw error;
      }
      const expectedContentTypes = normalizeExpectedContentTypes(options.expectedContentTypes);
      const response = await request(url, {
        ...options,
      ...
      if (
        ...
        && !matchesExpectedContentType(response.contentType, expectedContentTypes)
      ) {
        const error = new Error("Response content type does not satisfy the JSON contract.");
        error.code = "unexpected_content_type";
        error.contentType = response.contentType;
        error.expectedContentTypes = expectedContentTypes;
        error.finalUrl = response.finalUrl;
        throw error;
      }
      const text = decodeBytes(response.bytes, response.contentType, "utf-8");
      try {
        return {
          ...response,
