...
const DEFAULT_NOTICE_URL_PATTERN =
  /(mode=view|sMode=VIEW_FORM|iBrdContNo=|articleNo=|boardNo=|nttNo=|nttId=|idx=\d+|no=\d+|wr_id=\d+|boardSeq=\d+|b_idx=\d+|seq=\d+|uid=\d+|artclNo=|artclView\.do|notice-view\?id=|mod=document)/i;
...
function extractTitleFromNode(node) {
  ...
  const attributeTitle = ["title", "aria-label", "data-title", "data-subject"]
    .map((attribute) => cleanText(node.attr(attribute)))
    .find((value) => value && value.length > visible.length);
  return attributeTitle || visible;
}

function attachCandidateProvenance(item, provenance) {
  Object.defineProperty(item, "__crawlerCandidateProvenance", {
...
export function extractFromList(source, html) {
  ...
  const parserCounters = {
    filtered_navigation_anchor_count: 0,
    filtered_pagination_anchor_count: 0,
  };
  const finalize = (items = results) => attachOperationalListParserEvidence(
    items,
    collectOperationalListParserEvidence(source, html, items, parserCounters),
  );
  ...
  const pushResult = (node, index) => {
    ...
    let linkNode = null;
    if (itemRoot && source.linkSelector) {
      linkNode = itemRoot.is(source.linkSelector)
        ? itemRoot
        : itemRoot.find(source.linkSelector).first();
    } else if (itemRoot) {
      const defaultLinkSelector = "a[href], a[onclick], a[data-href], a[data-url], a[data-link]";
      linkNode = itemRoot.is(defaultLinkSelector)
        ? itemRoot
        : itemRoot.find(defaultLinkSelector).first();
    }
    ...
    const insideNavigationContainer = Boolean(
      activeLinkNode?.closest(OPERATIONAL_NAVIGATION_CONTAINER_SELECTOR).length
      || itemRoot?.closest(OPERATIONAL_NAVIGATION_CONTAINER_SELECTOR).length,
    );
    const insidePaginationContainer = Boolean(
      activeLinkNode?.closest(OPERATIONAL_PAGINATION_CONTAINER_SELECTOR).length
      || itemRoot?.closest(OPERATIONAL_PAGINATION_CONTAINER_SELECTOR).length,
    );
    if (insideNavigationContainer) parserCounters.filtered_navigation_anchor_count += 1;
    if (insidePaginationContainer) parserCounters.filtered_pagination_anchor_count += 1;
    // Structural containers are high-confidence non-detail links. A page=
    // parameter outside them remains eligible; many valid detail URLs use it.
    if (insideNavigationContainer || insidePaginationContainer) return;
    ...
    const configuredTitleNode = itemRoot && source.titleSelector
      ? itemRoot.is(source.titleSelector)
        ? itemRoot
        : itemRoot.find(source.titleSelector).first()
      : null;
    const titleRaw = extractTitleFromNode(configuredTitleNode)
      || extractTitleFromNode(activeLinkNode)
      || extractTitleFromNode(itemRoot);
    ...
    seen.add(noticeUrl);
    const candidateOrigin = source.listItemSelector
      ? "configured_selector"
      : itemRoot
        ? "common_board_selector"
        : "heuristic_anchor";
    results.push(attachCandidateProvenance({
      sourceId: source.sourceId,
      universitySlug: source.universitySlug,
      universityId: source.universityId,
      collegeId: source.collegeId,
      ...
      candidate_node_fingerprint: `${candidateOrigin}:${noticeUrl}`,
      // Generic extraction has no stable DOM path. Keep this explicit so
      // diagnostics never mistake the URL-based legacy fingerprint for one.
      dom_locator: null,
      inside_navigation_container: insideNavigationContainer,
      inside_pagination_container: insidePaginationContainer,
      matched_list_selector: source.listItemSelector || null,
      link_role: linkRole,
      duplicate_url_observed_in_navigation: false,
    }));
  };
