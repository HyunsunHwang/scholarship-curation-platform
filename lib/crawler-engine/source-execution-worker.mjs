...
async function run() {
  ...
  if (listAdapter) {
    ...
    const listObservations = trimItems(await listAdapter(source, {
      lookbackDays: config.lookbackDays,
      allowUndated: config.allowUndated,
      maxItems: config.maxItems,
    }), config.maxItems);
    const detectorOptions = candidateOptions(source, config);
    const { preliminaryCandidateResults, detailFetchPlan } =
      buildPreliminaryScholarshipCandidatePlan(listObservations, {
        detector: detectScholarshipCandidate,
        planner: buildDetailFetchPlan,
        detectorOptions,
        plannerOptions: { seenNoticeUrls: config.seenNoticeUrls ?? [] },
      });
    let notices = detailFetchPlan.fetch;
    ...
    const adapterResultStatus = listObservations.length > 0 ? "success" : "empty_observed";
    executionResult.result_status = adapterResultStatus;
    executionResult.observed_count = listObservations.length;
    executionResult.matched_count = candidateDetection.final_summary.candidate_count;
    executionResult.candidate_detection = candidateDetection;
    executionResult.item_summary = {
      ...
      successful_count: notices.length,
      failed_count: 0,
      cancelled_count: 0,
      preliminary_skip_count: detailFetchPlan.skip.length,
      resumed_skip_count: 0,
    };
    executionResult.attempt_history[0].status = adapterResultStatus;
    executionResult.attempt_history[0].reason_code = adapterResultStatus;
    executionResult.attempt_history[0].item_count = listObservations.length;
    executionResult.final_reason_code = adapterResultStatus;
