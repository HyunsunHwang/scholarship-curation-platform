...
async function run() {
  const { source, inventoryRows, completedWorkItemKeys, config } = workerData;
...
run().catch((error) => send({
  type: "error",
  error: { name: error?.name, message: String(error?.message ?? error), code: error?.code },
}));
