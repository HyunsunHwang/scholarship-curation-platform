...
export function createNoticeDocumentProcessor({
  ...
  return async ({ source, notice, detailHtml = "", signal }) => {
    const results = [];
      ...
      for (const asset of notice.attachment_metadata ?? notice.attachmentMetadata ?? []) {
        try {
          ...
          if (typeof inspectAsset === "function") {
            try { inspected = await inspectAsset(asset, { source, notice, signal }) ?? {}; } catch { inspected = {}; }
          }
          ...
          const fetched = await fetchAsset(asset, { source, notice, inspected, signal });
