...
function recordHttpCompletion({ hostname, durationMs, status, bytes = 0, failed = false, timeout = false }) {
...
export function applyCrawlerTelemetryEvent(event) {
  ...
  if (eventType === "http_started") {
    ...
    tracker.requests.set(eventId, { hostname });
...
export function telemetryHttpStarted({ url, attempt = 0 } = {}) {
  ...
  if (!state.session) {
    ...
    const event = emitTelemetryEvent({ event_type: "http_started", hostname, attempt: integer(attempt) });
    return event ? { worker_bridge: true, event_id: event.event_id, started, hostname, attempt } : null;
  }
...
export function telemetryHttpFinished(token, { status, bytes = 0, error, timeout = false } = {}) {
  ...
  recordHttpCompletion({ hostname: token.hostname, durationMs: duration, status, bytes, failed: Boolean(error), timeout });
}
