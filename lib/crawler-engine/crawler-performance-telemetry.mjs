...
export const CRAWLER_TELEMETRY_EVENT_SCHEMA_VERSION = "crawler-telemetry-event-v1";
...
const state = {
  ...
  listRequests: 0, detailRequests: 0,
  workerEvents: new Map(),
};
let telemetryEventSink = null;
...
function emitTelemetryEvent(event) {
  if (typeof telemetryEventSink !== "function") return null;
  const payload = {
    schema_version: CRAWLER_TELEMETRY_EVENT_SCHEMA_VERSION,
    event_id: randomUUID(),
    ...event,
  };
  try {
    telemetryEventSink(payload);
    return payload;
  } catch {
    return null;
  }
...
function workerEventState(workerId) {
  const safeWorkerId = String(workerId ?? "").replace(/[^A-Za-z0-9_.:-]/g, "_").slice(0, 160) || "unknown_worker";
  if (!state.workerEvents.has(safeWorkerId)) {
    state.workerEvents.set(safeWorkerId, { details: new Map(), requests: new Map() });
  }
  return { workerId: safeWorkerId, tracker: state.workerEvents.get(safeWorkerId) };
}

function safeTelemetryHostname(value) {
...
function recordHttpCompletion({
  ...
  const host = hostRecord(hostname || "invalid-host");
  host.active = Math.max(0, host.active - 1);
  host.request_count += 1;
  host.latencies.push(Math.max(0, Number(durationMs) || 0));
  state.activeRequests = Math.max(0, state.activeRequests - 1);
  state.completedRequests += 1;
  if (kind === "list") state.listRequests += 1;
  if (kind === "detail") state.detailRequests += 1;
  state.requestDurations.push(Math.max(0, Number(durationMs) || 0));
  state.responseBytes.push(Math.max(0, Number(bytes) || 0));
  if (failed) { state.requestFailure += 1; host.failure_count += 1; } else { state.requestSuccess += 1; host.success_count += 1; }
...
export function setCrawlerTelemetryEventSink(sink) {
  telemetryEventSink = typeof sink === "function" ? sink : null;
}

export function clearCrawlerTelemetryEventSink() {
  telemetryEventSink = null;
}

export function applyCrawlerTelemetryEvent(event) {
  if (!state.session || event?.schema_version !== CRAWLER_TELEMETRY_EVENT_SCHEMA_VERSION) return false;
  const eventType = String(event.event_type ?? "");
  const eventId = String(event.event_id ?? "");
  if (!eventId) return false;
  const { tracker } = workerEventState(event.worker_id);
  if (eventType === "detail_started") {
    if (tracker.details.has(eventId)) return false;
    tracker.details.set(eventId, { started: finite(event.started_monotonic_ms) ?? 0 });
    state.activeDetails += 1;
    return true;
  }
  if (eventType === "detail_finished") {
    const active = tracker.details.get(String(event.parent_event_id ?? ""));
    if (!active) return false;
    tracker.details.delete(String(event.parent_event_id));
    state.activeDetails = Math.max(0, state.activeDetails - 1);
    state.completedDetails += 1;
    state.detailDurations.push(Math.max(0, Number(event.duration_ms) || 0));
    return true;
  }
  if (eventType === "http_started") {
    if (tracker.requests.has(eventId)) return false;
    const hostname = safeTelemetryHostname(event.hostname);
    const kind = ["list", "detail"].includes(event.kind) ? event.kind : "other";
    tracker.requests.set(eventId, { hostname, kind });
    hostRecord(hostname).active += 1;
    state.activeRequests += 1;
    if (Number(event.attempt) > 0) {
      state.retryAttempts += 1;
      hostRecord(hostname).retry_count += 1;
    }
    return true;
  }
  if (eventType === "http_finished") {
    const parentEventId = String(event.parent_event_id ?? "");
    const active = tracker.requests.get(parentEventId);
    if (!active) return false;
    tracker.requests.delete(parentEventId);
    recordHttpCompletion({
      hostname: active.hostname,
      durationMs: event.duration_ms,
      status: optionalFinite(event.status),
      bytes: event.bytes,
  ...
  if (eventType === "retry_delay") {
    state.retryDelayMs += Math.max(0, Number(event.delay_ms) || 0);
    return true;
  }
  return false;
}

export function releaseCrawlerTelemetryWorker(workerId, reason = "worker_exit") {
  if (!state.session) return false;
  const { workerId: safeWorkerId, tracker } = workerEventState(workerId);
  const incomplete = tracker.details.size + tracker.requests.size;
  for (const request of tracker.requests.values()) {
    state.activeRequests = Math.max(0, state.activeRequests - 1);
    const host = hostRecord(request.hostname);
    host.active = Math.max(0, host.active - 1);
  }
  state.activeDetails = Math.max(0, state.activeDetails - tracker.details.size);
  state.workerEvents.delete(safeWorkerId);
  if (incomplete > 0) {
    state.session.warnings.push(
      `worker_telemetry_incomplete worker_id=${safeWorkerId} reason=${String(reason).replace(/[^A-Za-z0-9_.:-]/g, "_")}`,
    );
  }
  return incomplete > 0;
}

export function getCrawlerTelemetryCounters() {
  return {
    active_source_count: state.activeSources,
    active_detail_count: state.activeDetails,
    active_http_request_count: state.activeRequests,
    completed_source_count: state.completedSources,
    completed_detail_count: state.completedDetails,
    completed_request_count: state.completedRequests,
  };
}
...
export function telemetryDetailStarted() {
  if (!state.session) {
    const event = emitTelemetryEvent({ event_type: "detail_started", started_monotonic_ms: performance.now() });
    return event ? { worker_bridge: true, event_id: event.event_id, started: performance.now() } : null;
  }
...
export function telemetryDetailFinished(token) {
  if (!state.session) {
    if (token?.worker_bridge) emitTelemetryEvent({
      event_type: "detail_finished",
      parent_event_id: token.event_id,
      duration_ms: Math.max(0, performance.now() - token.started),
    });
    return;
  }
...
export function telemetryHttpStarted({ url, attempt = 0, kind = "other" } = {}) {
  let hostname = "invalid-host";
  try { hostname = new URL(String(url)).hostname.toLowerCase(); } catch {}
  const requestKind = ["list", "detail"].includes(kind) ? kind : "other";
  if (!state.session) {
    const started = performance.now();
    const event = emitTelemetryEvent({
      event_type: "http_started",
      hostname,
      attempt: integer(attempt),
      kind: requestKind,
    });
    return event
      ? { worker_bridge: true, event_id: event.event_id, started, hostname, attempt, kind: requestKind }
      : null;
  }
...
export function telemetryHttpFinished(token, { status, bytes = 0, error, timeout = false } = {}) {
  if (!state.session) {
    if (token?.worker_bridge) emitTelemetryEvent({
      event_type: "http_finished",
      parent_event_id: token.event_id,
      hostname: token.hostname,
      duration_ms: Math.max(0, performance.now() - token.started),
      status: optionalFinite(status),
      bytes: Math.max(0, Number(bytes) || 0),
      timeout: Boolean(timeout),
      failed: Boolean(error),
      attempt: integer(token.attempt),
      kind: token.kind,
    });
    return;
  }
...
export function telemetryRetryDelay(delayMs) {
  if (state.session) state.retryDelayMs += Math.max(0, Number(delayMs) || 0);
  else emitTelemetryEvent({ event_type: "retry_delay", delay_ms: Math.max(0, Number(delayMs) || 0) });
}
...
export async function finishCrawlerPerformanceTelemetry({ crawlerResult = null, error = null } = {}) {
  ...
  const observedNoticeCount = Number(
    crawlerResult?.report?.candidateDetection?.summary?.observed_list_item_count
    ?? crawlerResult?.report?.counts?.observed
    ?? crawlerResult?.report?.totals?.crawledCount
    ?? 0,
  ) || 0;
  const summary = {
    ...
    configuration: {
      ...
      source_registry: crawlerResult?.report?.sourceRegistry ?? null,
    ...
    performance: { sources_per_minute: perMinute(state.completedSources), details_per_minute: perMinute(state.completedDetails), requests_per_minute: perMinute(state.completedRequests),
      observed_notices_per_minute: perMinute(observedNoticeCount), ...distribution("source_duration", state.sourceDurations), ...distribution("detail_duration", state.detailDurations),
    ...
    reliability: { request_count: state.completedRequests, list_request_count: state.listRequests,
      detail_request_count: state.detailRequests,
      detail_request_skipped_count: Number(crawlerResult?.report?.candidateDetection?.summary?.detail_fetch_skipped_count) || 0,
      requests_avoided_by_preliminary_filter: Number(crawlerResult?.report?.candidateDetection?.summary?.requests_avoided_by_preliminary_filter) || 0,
      request_success_count: state.requestSuccess, request_failure_count: state.requestFailure,
    ...
    quality_metrics_source: {},
    warnings: [...session.warnings, ...(session.telemetryError ? [session.telemetryError] : [])],
  };
