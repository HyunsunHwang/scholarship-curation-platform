...
export function parseInlineSections(source = {}, html = "", options = {}) {
  ...
  const bodyBoundary = clean(source.sectionBodyBoundary)
    ...
    || "next_heading";
  const publishedDateSelector = clean(source.sectionPublishedDateSelector)
    || clean(options.publishedDateSelector);
  ...
  while (stack.length > 0 && traversedNodeCount < maxTraversedNodes) {
    ...
    if (element.is(titleSelector)) {
      ...
      currentSection = title && sections.length < maxSectionCount
        ? {
            ...
            links: [],
            publishedDateTexts: [],
            anchorId: clean(element.attr("id") || element.attr("data-section-id") || element.attr("data-id")),
          }
        : null;
    ...
    if (currentSection && publishedDateSelector && element.is(publishedDateSelector)) {
      const value = clean(element.text());
      if (value) currentSection.publishedDateTexts.push(value);
    }
  ...
  return Object.freeze({
    ...
    sections: sections.map((section) => Object.freeze({
      ...
      publishedDateTexts: [...new Set(section.publishedDateTexts)],
