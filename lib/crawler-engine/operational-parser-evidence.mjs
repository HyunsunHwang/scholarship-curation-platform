...
import { parseInlineSections } from "./inline-section-parser.mjs";
...
// Container membership is the only structural evidence that permits the
// generic parser to discard a link before it becomes a candidate. A `page=`
// query parameter alone is evidence for pagination diagnostics, not proof
// that a detail link is pagination.
export const OPERATIONAL_PAGINATION_CONTAINER_SELECTOR = [
  ".pagination",
  ".paging",
  ".paginate",
  ".page-navigation",
...
const PAGINATION_EVIDENCE_SELECTOR = [
  OPERATIONAL_PAGINATION_CONTAINER_SELECTOR,
  "a[href*='page=']",
  "a[href*='pageNo=']",
  "a[href*='pageIndex=']",
].join(",");
...
function matchesConfiguredNoticePattern(value, pattern) {
  const configured = clean(pattern);
  if (!configured) return false;
  try {
    return new RegExp(configured, "i").test(value);
  } catch {
    return false;
  }
}
...
export function classifyOperationalLinkRole({ href = "", text = "", source = {} } = {}) {
  ...
  if (/^(?:mailto:|tel:)/i.test(rawHref)) return "supporting_reference";
  if (!rawHref || /^#/.test(rawHref)) return "same_origin_navigation";
  const resolved = resolveHttpUrl(href, source);
  if (!resolved) return "unknown_external";
  const label = clean(text);
  let url;
  try { url = new URL(resolved); } catch { return "unknown_external"; }
  let sourceOrigin = "";
  try { sourceOrigin = new URL(source.listUrl || source.baseUrl).origin.toLowerCase(); } catch {}
  const sameOrigin = Boolean(sourceOrigin) && url.origin.toLowerCase() === sourceOrigin;
  ...
  if (samePageUrl(resolved, source.listUrl)
    || /(?:^|\/)(?:index|main|home)(?:\.|\/|$)/i.test(url.pathname)
    || /(?:home|menu|previous|next|list|\ud648|\uba54\ub274|\uc774\uc804|\ub2e4\uc74c|\ubaa9\ub85d)/i.test(label)) {
    return "same_origin_navigation";
  }
  if (matchesConfiguredNoticePattern(resolved, source.noticeUrlPattern)) {
  ...
  return "same_origin_unknown";
}

function collectContentTopologyEvidence($, source) {
  const parsedSections = parseInlineSections(source, $.html(), {
    titleSelector: clean(source.sectionTitleSelector) || "h1,h2,h3,h4,h5,h6",
  });
  const headings = parsedSections.sections
    .filter((section) => section.title.length >= 4);
  const substantive = headings.filter((section) => section.body.length >= 40);
  const roles = {
    same_origin_detail: 0,
    same_origin_navigation: 0,
    same_origin_unknown: 0,
    cross_origin_authoritative_detail: 0,
    attachment: 0,
    application_form: 0,
    supporting_reference: 0,
    unknown_external: 0,
  };
  for (const section of parsedSections.sections) {
    for (const link of section.links) {
      const role = classifyOperationalLinkRole({
        href: link.href,
        text: link.label,
        source,
      });
  ...
  const sameOriginLinkCount = roles.same_origin_detail
    + roles.same_origin_navigation
    + roles.same_origin_unknown;
  ...
  const declaredInline = clean(source.contentMode) === "inline_sections";
  const independentDetailUrlCount = roles.same_origin_detail
    + (declaredInline ? 0 : roles.cross_origin_authoritative_detail);
  const inlineBodyCharCount = substantive.reduce((sum, section) => sum + section.body.length, 0);
  const headingNoticeSignalCount = substantive.filter((section) =>
    /(?:20\d{2}|\ub300\ud559\uc0dd|\uc7a5\ud559|\uacf5\uc9c0|\ubaa8\uc9d1|\uc548\ub0b4|\uacb0\uacfc|scholarship|notice|announcement|application|result)/i
      .test(section.title)).length;
  const sectionDensity = headings.length > 0 ? substantive.length / headings.length : 0;
  // Repeated substantial sections and the absence of independent detail URLs
  // are the core topology evidence. External links strengthen the inference but
  // are not required: some valid inline notice pages contain text only.
  const inlineDetected = substantive.length >= 2
    && inlineBodyCharCount >= 160
    && independentDetailUrlCount === 0
    && (declaredInline
      || headingNoticeSignalCount >= 2
      || (headingNoticeSignalCount >= 1 && sectionDensity >= 0.5)
      || externalResourceLinkCount >= 1);
  return {
    heading_candidate_count: headings.length,
    heading_with_body_count: substantive.length,
    heading_notice_signal_count: headingNoticeSignalCount,
    inline_notice_section_count: inlineDetected ? substantive.length : 0,
    inline_body_char_count: inlineBodyCharCount,
    inline_section_traversed_node_count: parsedSections.traversed_node_count,
    inline_section_traversal_truncated: parsedSections.traversal_truncated,
    ...
    detail_like_same_origin_link_count: roles.same_origin_detail,
    same_origin_navigation_link_count: roles.same_origin_navigation,
    same_origin_unknown_link_count: roles.same_origin_unknown,
    cross_origin_authoritative_detail_link_count: roles.cross_origin_authoritative_detail,
    external_resource_link_count: externalResourceLinkCount,
    attachment_link_count: roles.attachment,
    application_form_link_count: roles.application_form,
    supporting_reference_link_count: roles.supporting_reference,
    unknown_external_link_count: roles.unknown_external,
    navigation_link_count: roles.same_origin_navigation,
...
export function collectOperationalListParserEvidence(source = {}, html = "", parsedItems = [], parserCounters = {}) {
  const $ = loadHtml(String(html ?? ""));
  const configuredSelector = clean(source.listItemSelector);
  const profileEvidence = source?.__crawlerListParserProfile ?? null;
  const profileApplied = Array.isArray(profileEvidence?.appliedFields)
    && profileEvidence.appliedFields.length > 0;
  ...
  const detailItems = items.filter((item) =>
    !clean(item?.inlineSectionId ?? item?.inline_section_id));
  ...
  const contaminatedLeakCount = detailItems.filter((item) => {
  ...
  const paginationEvidenceCount = $(PAGINATION_EVIDENCE_SELECTOR).length;
  ...
  const topology = collectContentTopologyEvidence($, source);
  // Generic extraction structurally filters navigation and pagination anchors
  // before candidate creation. Therefore a selected-candidate leak should be
  // zero for that path; a URL duplicated in a sidebar is only overlap evidence.
  const candidateNavigationLeakCount = detailItems.filter((item) =>
    item?.__crawlerCandidateProvenance?.inside_navigation_container === true
    || item?.__crawlerCandidateProvenance?.inside_pagination_container === true).length;
  ...
  return Object.freeze({
    ...
    list_parser_profile: clean(profileEvidence?.profileId) || null,
    ...
    resolved_detail_url_count: detailItems.filter((item) => clean(item?.noticeUrl)).length,
    valid_detail_url_count: detailItems.filter((item) => isHttpUrl(item?.noticeUrl)).length,
    inline_section_identity_count: items.length - detailItems.length,
    ...
    // Compatibility alias: this now means a selected-node leak, never a URL
    // overlap. Consumers needing historical overlap use the explicit field.
    contaminated_candidate_leak_count: candidateNavigationLeakCount,
    candidate_navigation_leak_count: candidateNavigationLeakCount,
    navigation_url_overlap_count: contaminatedLeakCount,
    filtered_navigation_anchor_count: Number(parserCounters.filtered_navigation_anchor_count) || 0,
    filtered_pagination_anchor_count: Number(parserCounters.filtered_pagination_anchor_count) || 0,
...
export function aggregateOperationalListParserEvidence(evidenceRows = []) {
  ...
  return Object.freeze({
    ...
    list_parser_profile: sortedUnique(rows.map((row) => row.list_parser_profile))[0] ?? null,
    profile_applied: rows.some((row) => row.profile_applied === true),
    ...
    inline_section_identity_count: sumObserved(rows, "inline_section_identity_count"),
    ...
    candidate_navigation_leak_count: sumObserved(rows, "candidate_navigation_leak_count"),
    navigation_url_overlap_count: sumObserved(rows, "navigation_url_overlap_count"),
    filtered_navigation_anchor_count: sumObserved(rows, "filtered_navigation_anchor_count"),
    filtered_pagination_anchor_count: sumObserved(rows, "filtered_pagination_anchor_count"),
    pagination_evidence_count: paginationEvidenceCount,
    ...
    heading_notice_signal_count: sumObserved(rows, "heading_notice_signal_count"),
    ...
    inline_section_traversed_node_count: sumObserved(rows, "inline_section_traversed_node_count"),
    inline_section_traversal_truncated: rows.some((row) => row.inline_section_traversal_truncated === true),
    ...
    same_origin_navigation_link_count: sumObserved(rows, "same_origin_navigation_link_count"),
    same_origin_unknown_link_count: sumObserved(rows, "same_origin_unknown_link_count"),
    cross_origin_authoritative_detail_link_count:
      sumObserved(rows, "cross_origin_authoritative_detail_link_count"),
    ...
    application_form_link_count: sumObserved(rows, "application_form_link_count"),
    supporting_reference_link_count: sumObserved(rows, "supporting_reference_link_count"),
    unknown_external_link_count: sumObserved(rows, "unknown_external_link_count"),
    navigation_link_count: sumObserved(rows, "navigation_link_count"),
