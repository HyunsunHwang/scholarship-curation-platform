...
import { buildImmutableCrawlerHandoff, normalizeCrawlerNotice } from "./crawler-handoff.mjs";
...
export async function runCommonCrawlerSource({
  ...
  // Inline-section and API adapters can provide authoritative detail content in
  // the list observation itself. Those observations still need normalization
  // and final candidate detection, but must not be converted into HTTP detail
  // requests by the planner.
  const processingItems = source.detailContentAlreadyAvailable === true
    ? items
    : plannedItems;
  ...
  const scheduledItems = processingItems.filter((item) => {
  ...
  const processItem = async (item, { skipDocumentProcessing = false } = {}) => {
    ...
    try {
      const extractedAttachmentMetadata = strategy.extractAttachmentMetadata
        ? strategy.extractAttachmentMetadata({ source, item, detail })
        : detail.attachmentMetadata ?? [];
      const attachmentMetadata = Array.isArray(extractedAttachmentMetadata) && extractedAttachmentMetadata.length > 0
        ? extractedAttachmentMetadata
        : Array.isArray(item.attachmentMetadata) && item.attachmentMetadata.length > 0
          ? item.attachmentMetadata
          : Array.isArray(item.attachment_metadata) ? item.attachment_metadata : [];
  ...
  const candidateDetection = candidatePipelineEnabled
    ? finalizeScholarshipCandidateDetection({
        ...
        detailContentAlreadyAvailable: source.detailContentAlreadyAvailable === true,
      })
    : null;
  const finalNotices = notices.map((notice, index) => normalizeCrawlerNotice(notice, {
    source: { ...source, sourceId },
    candidateResult: candidateDetection?.final_candidate_results?.[index] ?? null,
  }));
  ...
  // A diagnostic probe is deliberately outside the candidate/detail work set.
  // Keep its failure evidence, but do not let one stale navigation URL turn an
  // otherwise successful source into a partial crawl result.
  const failedItemCount = candidateFailedItemCount;
  const resumedSkipCount = processingItems.length - scheduledItems.length;
  ...
  const checkpointWorkItemKeys = items
    .map((item) => workItemIdentity?.(sourceKey, item))
    .filter(Boolean);
  ...
  return {
    ...
    matched_count: candidateDetection?.final_summary?.candidate_count ?? finalNotices.length,
    timeout: false,
    item_summary: {
      observed_list_item_count: items.length,
      eligible_count: processingItems.length,
      ...
      checkpoint_work_item_unique_count: new Set(checkpointWorkItemKeys).size,
    },
...
export function isRetryableCrawlerResult(result) {
  if (isContentContractErrorCode(result?.error_code)) return false;
...
export async function runCommonCrawler({
  ...
  return {
    ...
    immutable_handoff: buildImmutableCrawlerHandoff({
      sourceResults: normalizedSourceResults,
