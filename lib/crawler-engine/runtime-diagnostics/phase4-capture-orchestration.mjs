...
export { PHASE4_CAPTURE_ARTIFACT_SCHEMA_VERSION, PHASE4_CAPTURE_CONTRACT_VERSION, PHASE4_CAPTURE_SCHEMA_VERSION };
...
export async function createPhase4CaptureJournal({ checkpointPath, resume, runIdentity, contractFingerprint, sourceIds }) {
  ...
  // A shared journal object is updated by bounded concurrent workers. Serialize
  // every transition so a later attempt can never overwrite a terminal write.
  let mutationTail = Promise.resolve();
  let fatalError = null;
  const mutation = (mutator) => {
    if (fatalError) return Promise.reject(checkpointError("phase4_journal_session_failed", "Phase 4 journal session is in a fatal state."));
    const operation = mutationTail.then(async () => {
      if (fatalError) throw checkpointError("phase4_journal_session_failed", "Phase 4 journal session is in a fatal state.");
      await mutator();
      validatePhase4CaptureJournal(journal, { runIdentity, contractFingerprint, sourceIds });
      await save();
    });
    mutationTail = operation.catch((error) => {
      // Transition conflicts are fail-closed for that caller but do not make an
      // already durable journal ambiguous. A failed atomic write does.
      if (error?.code === "phase4_checkpoint_atomic_write_failed") fatalError ??= error;
    });
    return operation;
  };
  return {
    ...
    hasTerminal(sourceId) { return Object.hasOwn(journal.terminal_artifacts, sourceId); },
    terminalEntry(sourceId) { return journal.terminal_artifacts[sourceId] ?? null; },
    attemptEntry(sourceId) { return journal.attempts[sourceId] ?? null; },
    async recordArtifact(sourceId, metadata) {
      return mutation(async () => {
        const next = stable(metadata); const current = journal.terminal_artifacts[sourceId];
        if (current && canonicalJson(current) !== canonicalJson(next)) throw checkpointError("phase4_journal_terminal_conflict", `${sourceId}: terminal artifact conflicts with existing terminal state.`);
        if (current) return;
        delete journal.attempts[sourceId];
        journal.terminal_artifacts[sourceId] = next;
      });
    },
    async recordAttempt(sourceId, metadata) {
      return mutation(async () => {
        if (Object.hasOwn(journal.terminal_artifacts, sourceId)) throw checkpointError("phase4_journal_terminal_already_committed", `${sourceId}: terminal artifact is already committed.`);
        journal.attempts[sourceId] = stable(metadata);
      });
    },
    async flush() { await mutationTail; if (fatalError) throw checkpointError("phase4_journal_session_failed", "Phase 4 journal session is in a fatal state."); },
    snapshot() { if (fatalError) throw checkpointError("phase4_journal_session_failed", "Phase 4 journal session is in a fatal state."); return structuredClone(journal); },
  };
}

/** Validates the Phase 4 journal; this is intentionally distinct from the generic crawler checkpoint. */
export function validatePhase4CaptureJournal(journal, { runIdentity, contractFingerprint, sourceIds } = {}) {
  if (!journal || typeof journal !== "object" || Array.isArray(journal) ||
      journal.schema_version !== PHASE4_CAPTURE_CHECKPOINT_SCHEMA_VERSION ||
      journal.run_identity !== runIdentity || journal.contract_fingerprint !== contractFingerprint) {
    throw checkpointError("phase4_checkpoint_contract_mismatch", "Phase 4 capture journal contract does not match this run.");
  }
  const expected = [...sourceIds].sort();
  const ids = journal.source_ids;
  if (!Array.isArray(ids) || new Set(ids).size !== ids.length || JSON.stringify(ids) !== JSON.stringify(expected)) {
    throw checkpointError("phase4_checkpoint_invalid", "Phase 4 capture journal source set is invalid.");
  }
  const terminal = journal.terminal_artifacts ?? {};
  const attempts = journal.attempts ?? {};
  if (!terminal || typeof terminal !== "object" || Array.isArray(terminal) || !attempts || typeof attempts !== "object" || Array.isArray(attempts)) {
    throw checkpointError("phase4_checkpoint_invalid", "Phase 4 capture journal entries are invalid.");
  }
  for (const sourceId of [...Object.keys(terminal), ...Object.keys(attempts)]) {
    if (!expected.includes(sourceId) || (Object.hasOwn(terminal, sourceId) && Object.hasOwn(attempts, sourceId))) {
      throw checkpointError("phase4_checkpoint_invalid", "Phase 4 journal contains contradictory source entries.");
    }
  }
  return { ...journal, source_ids: ids, terminal_artifacts: terminal, attempts };
}

export function buildPhase4CaptureExecutionContract({ sources, selected = sources, sourceRegistry, transportRegistry, registry = transportRegistry, resolvedTransportPolicies, policies = resolvedTransportPolicies, controlConfigs, sourceConcurrency, hostConcurrency, timeoutMs, retryCount, codeSha }) {
  return stable({
...
export function fingerprintPhase4CaptureExecutionContract(contract) { return sha256(canonicalJson(contract)); }
export function derivePhase4CaptureRunIdentity(contractFingerprint) { return `phase4-capture-${contractFingerprint}`; }

/** Pure terminal-artifact builder. Transport terminal states retain root provenance. */
export function buildPhase4CaptureArtifact({ result, runIdentity, contractFingerprint, contract }) {
  return stable({
    ...
    // Capture success and invalid-content retain provenance in capture. Transport
    // terminal states have no capture object, so preserve their evidence at root.
    transport_evidence: result.transport_evidence ?? result.capture?.transport_evidence ?? null,
...
export async function runPhase4SameHtmlCapture({
  ...
  const contract = buildPhase4CaptureExecutionContract({ selected, sourceRegistry, registry, policies, controlConfigs, sourceConcurrency, hostConcurrency, timeoutMs, retryCount, codeSha });
  const contractFingerprint = fingerprintPhase4CaptureExecutionContract(contract);
  const runIdentity = derivePhase4CaptureRunIdentity(contractFingerprint);
  ...
  const results = await boundedMap(selected, sourceConcurrency, async (source) => {
    ...
    const artifact = buildPhase4CaptureArtifact({ result, runIdentity, contractFingerprint, contract });
    // Validate the pure builder before invoking a writer. A malformed artifact is a
    // programming error, distinct from a filesystem publish/readback failure.
    validatePhase4CaptureArtifact(artifact, { expected: { sourceId: source.sourceId, runIdentity, contractFingerprint } });
    let metadata;
    try {
      ...
      validatePhase4CaptureArtifact(artifact, { expected: { sourceId: source.sourceId, runIdentity, contractFingerprint }, metadata: validatePhase4ArtifactMetadata(metadata, { sourceId: source.sourceId, runIdentity, contractFingerprint }) });
    ...
    try {
      ...
      throw checkpointError("phase4_journal_sync_failure", `${source.sourceId}: terminal artifact was published but the journal could not be synchronized.`);
    }
    try {
      ...
      throw checkpointError("phase4_checkpoint_sync_failure", `${source.sourceId}: terminal journal entry was committed but the generic checkpoint could not be synchronized.`);
    }
