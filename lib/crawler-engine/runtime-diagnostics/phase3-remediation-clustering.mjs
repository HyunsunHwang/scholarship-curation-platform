...
function assertSameCaptureEvidence(evidence, sourceId) {
  ...
  if (sourceFields.some((value) => clean(value) !== expectedSource)) throw new Error(`${sourceId}: same-capture source provenance mismatch.`);
  ...
  if (captureIds.some((value) => clean(value) !== clean(evidence.capture_id))) throw new Error(`${sourceId}: same-capture capture ID mismatch.`);
  const hash = clean(evidence.capture?.html_sha256);
  if (!/^[a-f0-9]{64}$/i.test(hash) || [evidence.control?.html_sha256, evidence.treatment?.html_sha256, evidence.candidate_comparison?.same_html_sha256].some((value) => clean(value) !== hash)) throw new Error(`${sourceId}: same-capture hash provenance is invalid.`);
  for (const field of ["requested_url", "final_url"]) if ([evidence.capture?.[field], evidence.control?.[field], evidence.treatment?.[field]].some((value) => clean(value) !== clean(evidence.capture?.[field]))) throw new Error(`${sourceId}: same-capture ${field} provenance mismatch.`);
...
function structuralEvidence(evidence) {
  ...
  const rawDomEvidence = {
    ...
    candidate_locators: uniqueSorted(candidates.map((candidate) => clean(candidate.provenance?.candidate_node_fingerprint ?? candidate.dom_locator))),
    candidate_origins: uniqueSorted(candidates.map((candidate) => clean(candidate.provenance?.candidate_origin ?? candidate.parser_strategy))),
    link_roles: uniqueSorted(candidates.map((candidate) => clean(candidate.provenance?.link_role ?? candidate.classification_evidence))),
  };
  const candidateFamilies = candidates.map((candidate) => domFamily({ selector: rawDomEvidence.matched_list_selector ?? rawDomEvidence.list_item_selector, locator: candidate.provenance?.candidate_node_fingerprint ?? candidate.dom_locator, parser }));
  const normalizedDomSignature = {
    ...
    candidate_locator_templates: uniqueSorted(rawDomEvidence.candidate_locators.map(normalizedLocator)),
