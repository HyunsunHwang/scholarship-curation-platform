...
function assertContained(parentPath, childPath, code) { const parent = path.resolve(parentPath); const child = path.resolve(childPath); const relative = path.relative(parent, child); if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) fail(code, "Phase 4 private artifact path escapes its expected parent."); }
...
export function validatePhase4PrivateArtifactRoot({ repositoryRoot, privateArtifactRoot, runIdentity, mode = "preflight", createRunDirectory = false } = {}) {
  ...
  const base = path.join(privateRoot, "phase4-capture"); if (fs.existsSync(base)) { const stat = fs.lstatSync(base); if (stat.isSymbolicLink()) fail("private_artifact_base_symlink", "Phase 4 capture base may not be a symlink."); if (!stat.isDirectory()) fail("private_artifact_base_not_directory", "Phase 4 capture base must be a directory."); assertContained(fs.realpathSync(privateRoot), fs.realpathSync(base), "private_artifact_base_escape"); }
  const runDirectory = path.join(base, runIdentity); const exists = fs.existsSync(runDirectory);
  ...
  if (createRunDirectory) { if (mode === "resume") fail("private_artifact_resume_contract_invalid", "Resume cannot create a run directory."); fs.mkdirSync(base, { recursive: true }); if (fs.lstatSync(base).isSymbolicLink()) fail("private_artifact_base_symlink", "Phase 4 capture base may not be a symlink."); assertContained(fs.realpathSync(privateRoot), fs.realpathSync(base), "private_artifact_base_escape"); fs.mkdirSync(runDirectory, { recursive: false }); }
