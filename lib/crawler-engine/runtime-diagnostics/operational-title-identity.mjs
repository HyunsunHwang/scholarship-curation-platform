...
function truncatedTitlePrefix(value) {
  ...
  return normalizeOperationalTitleForIdentity(raw.replace(/(?:\.\.\.|…)$/u, ""));
}
...
export function verifyOperationalDetailTitleIdentity(listTitle, detailTitleCandidates = []) {
  ...
  // A server-rendered list can deliberately truncate a long title. This is a
  // narrow identity rule: a visible ellipsis, a substantial normalized prefix,
  // and exactly one detail-title candidate must all agree. It is not a general
  // startsWith fallback.
  const truncatedPrefix = truncatedTitlePrefix(listTitle);
  if (truncatedPrefix.length >= 12) {
    const matchingCandidates = [...new Map(
      detailTitleCandidates
        .map((rawCandidate) => clean(rawCandidate))
        .filter(Boolean)
        .filter((candidate) => normalizeOperationalTitleForIdentity(candidate).startsWith(truncatedPrefix))
        .map((candidate) => [normalizeOperationalTitleForIdentity(candidate), candidate]),
    ).values()];
    if (matchingCandidates.length === 1) {
      return {
        verified: true,
        status: "title_match",
        comparison_mode: "list_title_truncated_prefix",
        matched_title: matchingCandidates[0],
      };
    }
  }
