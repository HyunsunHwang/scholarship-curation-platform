...
export const PHASE4B_CONTROL_COMMIT = "b9bf5002703e0e131bade45a88f561bb46cb389a";
...
function ensureHistoricalWorktree(repositoryRoot, worktreePath, commit) {
  if (fs.existsSync(worktreePath)) {
    if (runGit(worktreePath, ["rev-parse", "HEAD"]) !== commit) throw new Error("Historical control worktree commit mismatch.");
    return worktreePath;
  }
  fs.mkdirSync(path.dirname(worktreePath), { recursive: true });
  execFileSync("git", ["-C", repositoryRoot, "worktree", "add", "--detach", worktreePath, commit], { stdio: "pipe" });
  return worktreePath;
}

async function loadHistoricalRuntime({ repositoryRoot, worktreePath, controlCommit }) {
  const directory = ensureHistoricalWorktree(repositoryRoot, worktreePath, controlCommit);
  const loader = await import(`${pathToFileURL(path.join(directory, "lib/notice-source-manifest-loader.mjs")).href}?phase4b=${controlCommit}`);
  const parser = await import(`${pathToFileURL(path.join(directory, "lib/crawler-engine/crawler-list-parser.mjs")).href}?phase4b=${controlCommit}`);
  const profiles = await import(`${pathToFileURL(path.join(directory, "lib/crawler-engine/list-parser-profiles.mjs")).href}?phase4b=${controlCommit}`);
  return { directory, sources: loader.loadNoticeSourceManifestRegistry().sources, parser: parser.extractFromList, applyProfile: profiles.applyListParserProfile };
}
...
export async function buildPhase4BOfflinePilot({
  repositoryRoot, rawCaptureDirectory, pilotReportPath, controlCommit = PHASE4B_CONTROL_COMMIT,
  worktreePath = path.join(repositoryRoot, ".tmp", `phase4b-control-${controlCommit.slice(0, 12)}`),
} = {}) {
  ...
  const historical = await loadHistoricalRuntime({ repositoryRoot: root, worktreePath, controlCommit });
