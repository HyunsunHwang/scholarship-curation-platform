...
import { buildPhase2SameHtmlComparison, preparePhase2SameHtmlInputs, resolvePhase2ParserConfig, runPhase2ControlParser, runPhase2TreatmentParser, validatePhase2ComparisonInputs, comparePhase2ParserResults, validatePhase2ComparisonOutput } from "./phase2-same-html-comparison.mjs";
...
export function loadLegacyPhase4PilotCapture({ sourceId, artifactPath, reportSource }) {
  ...
  return stable({
    ...
    inventory: {
      source_id: sourceId, raw_capture_status: "raw_capture_verified", capture_status: "capture_success", requested_url: capture.requested_url,
...
function typedFailure(sourceId, stage, error) {
  const statuses = { control_config: "control_config_missing", treatment_config: "treatment_config_missing", control_profile: "control_profile_failure", treatment_profile: "treatment_profile_failure", control_parser: "control_parser_failure", treatment_parser: "treatment_parser_failure", comparison_input: "comparison_input_invariant_failure", comparison: "comparison_execution_failure", comparison_output: "comparison_output_invariant_failure" };
  return { source_id: sourceId, execution_status: statuses[stage] ?? "comparison_execution_failure", failed_stage: stage, error_code: clean(error?.code) || "stage_throw", error_summary: clean(error?.message).replace(/[A-Za-z]:\\[^\s]+|\/[^\s]+/g, "[redacted-path]").slice(0, 240) || "Sanitized deterministic failure.", next_phase_queue: "evidence_reconciliation", blocking_reason: statuses[stage] ?? "comparison_execution_failure" };
}
  ...
  await withOfflineNetworkGuard(async () => {
    for (const row of verified) {
      ...
      if (!historicalSource || !treatmentSource) {
        reconciliations.push({ source_id: row.source_id, execution_status: "control_config_missing", failed_stage: "control_config", error_code: "source_config_absent", error_summary: "Historical or treatment source configuration is absent.", next_phase_queue: "evidence_reconciliation", blocking_reason: "source_config_absent" });
        continue;
      }
      const provenance = controlProvenance({ repositoryRoot: root, source: historicalSource, controlCommit }); let capture; let controlSource; let treatmentSourceResolved; let control; let treatment; let comparison;
      try { capture = preparePhase2SameHtmlInputs(row.capture); } catch (error) { reconciliations.push(typedFailure(row.source_id, "comparison_input", error)); continue; }
      try { controlSource = resolvePhase2ParserConfig({ config: historicalSource, label: "control", capture, applyProfile: historical.applyProfile }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "control_profile", error)); continue; }
      try { treatmentSourceResolved = resolvePhase2ParserConfig({ config: treatmentSource, label: "treatment", capture, applyProfile: applyListParserProfile }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "treatment_profile", error)); continue; }
      try { control = runPhase2ControlParser({ parser: ({ source, html }) => historical.parser(source, html), source: controlSource, capture }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "control_parser", error)); continue; }
      try { treatment = runPhase2TreatmentParser({ parser: ({ source, html }) => extractFromList(source, html), source: treatmentSourceResolved, capture }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "treatment_parser", error)); continue; }
      try { validatePhase2ComparisonInputs({ capture, control, treatment }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "comparison_input", error)); continue; }
      try { comparison = comparePhase2ParserResults({ capture, control, treatment }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "comparison", error)); continue; }
      try { validatePhase2ComparisonOutput({ capture, control, treatment, comparison }); } catch (error) { reconciliations.push(typedFailure(row.source_id, "comparison_output", error)); continue; }
      const evidence = { schema_version: "phase2-same-html-comparison-v1", source_id: capture.source_id, capture_id: capture.capture_id, capture: { ...capture, html: undefined, html_bytes: undefined }, control, treatment, candidate_comparison: comparison };
      comparisons.push({ ...sanitizedComparison(evidence), control_provenance: { ...provenance, reconstruction_result_hash: sha256(canonical(control)) } });
    }
  });
...
export function phase4BReportFiles(output) {
  ...
  const reconciliation = { schema_version: PHASE4B_OFFLINE_VERSION, control_reference_commit: output.control_reference_commit, sources: [...output.comparisons.map((row) => ({ source_id: row.source_id, execution_status: "comparison_ready", failed_stage: null, error_code: null, error_summary: null, control_provenance: row.control_provenance })), ...output.reconciliations].sort((a, b) => a.source_id.localeCompare(b.source_id)) };
