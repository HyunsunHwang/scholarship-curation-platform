  ...
  await withOfflineNetworkGuard(async () => {
    for (const row of verified) {
      ...
      try {
        ...
        reconciliations.push({ source_id: row.source_id, execution_status: "comparison_execution_failure", failed_stage: "comparison", error_code: clean(error?.code) || "comparison_throw", error_summary: clean(error?.message).replace(/[A-Za-z]:\\[^\s]+|\/[^\s]+/g, "[redacted-path]").slice(0, 240) || "Sanitized comparison execution failure.", next_phase_queue: "evidence_reconciliation", blocking_reason: "comparison_execution_failure" });
      }
    }
  });
