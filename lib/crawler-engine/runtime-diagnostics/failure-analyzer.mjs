...
const HTTP_PROTOCOL_CODES = new Set(["INVALID_HTTP_RESPONSE"]);
const CONTENT_CONTRACT_ERROR_CODES = new Set([
  "json_decode_error",
  "json_shape_mismatch",
  "unexpected_content_type",
]);
const NON_RETRYABLE_TRANSPORT_CODES = new Set([...TLS_CERTIFICATE_CODES, ...HTTP_PROTOCOL_CODES, "ENOTFOUND"]);
...
export function isContentContractErrorCode(value) {
  return CONTENT_CONTRACT_ERROR_CODES.has(clean(value).toLowerCase());
}
...
export function classifyTransportErrorCode(value) {
  ...
  if (HTTP_PROTOCOL_CODES.has(code)) return "http_protocol";
  return "unknown";
}
...
export function extractSafeCrawlerErrorEvidence(error) {
  ...
  for (let depth = 0; current && depth < 4; depth += 1) {
    const message = clean(`${current.message ?? ""} ${current.cause?.message ?? ""}`);
    // Some OpenSSL failures arrive through Undici with the unhelpful
    // UNSPECIFIED code. Preserve the stable certificate category without
    // retaining the raw error message in the report.
    if (/ee certificate key too weak|certificate key too weak/i.test(message)) {
      return {
    ...
    if (/invalid header token|response does not match the http\/1\.1 protocol/i.test(message)) {
      return {
...
export function classifyCrawlerFailure(error, fallback = "network_error", { isCancellation } = {}) {
  ...
  if (isContentContractErrorCode(error?.code)) return "parser_error";
  return fallback;
}
