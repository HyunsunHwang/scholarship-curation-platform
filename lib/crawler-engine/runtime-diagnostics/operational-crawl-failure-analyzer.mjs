...
function phase2Recall(candidateComparison, analysisCodes) {
  ...
  const values = Object.fromEntries(fields.map((field) => [field, observedNumber(candidateComparison[field])]));
...
export function analyzePhase2ParserRemediation(input = {}) {
  ...
  const parserContract = {
    ...
    candidate_navigation_leak_count: count(metrics.candidate_navigation_leak_count),
  };
  ...
  let phase2Status = "manual_review_required";
  let nextPhaseQueue = "fixture_evidence";
  let blockingReason = recall.verified ? "insufficient_parser_or_identity_evidence" : "candidate_recall_unverified";
  if (runtimeAccessibility.shared_external_failure) {
    phase2Status = "source_unreachable";
    nextPhaseQueue = "runtime_accessibility";
    ...
    nextPhaseQueue = "adapter_contract";
  ...
  } else if (recall.verified
    && clean(treatment.runtime_result_status) === "success"
    && parserContract.candidate_navigation_leak_count === 0
    && detailIdentity.verified_count >= 3) {
    if (input.listUrlChanged === true || input.list_url_changed === true) {
      phase2Status = "list_url_corrected";
  ...
  return {
    ...
    next_action: phase2Status === "manual_review_required" ? "collect_fixture_evidence" : "none",
