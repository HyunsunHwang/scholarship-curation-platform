...
function collectNoticeMetrics(notices, adapterEvidence) {
...
function buildMetrics({ result, parserEvidence, filterMetrics, notices, matchedCount, adapterEvidence }) {
  ...
  const noticeMetrics = collectNoticeMetrics(notices, adapterEvidence);
  return {
...
function inferProfiles({ result, parserEvidence, metrics, adapterEvidence }) {
  ...
  if (parserEvidence?.configured_list_selector && count(parserEvidence?.selector_match_count) === 0) {
...
function inferOperationalCodes({ result, parserEvidence, filterMetrics, metrics, profiles, adapterEvidence }) {
  const codes = new Set();
  const adapterAuthoritative = adapterEvidence?.adapter_capability_verified === true
    && adapterEvidence?.adapter_provides_authoritative_detail === true;
  ...
  if (configuredSelectorObserved && count(parserEvidence.selector_match_count) === 0) {
  ...
  if (count(metrics.list_candidate_count) > 0 && count(metrics.valid_detail_url_count) === 0) {
  ...
  if (count(metrics.detail_fetch_failure_count) > 0) codes.add(OPERATIONAL_CRAWL_CODES.DETAIL_FETCH_FAILED);
  ...
  ].includes(profile)) && adapterEvidence?.adapter_capability_verified !== true;
  ...
  if ((parserObserved && metrics.list_candidate_count === 0 && !codes.has(OPERATIONAL_CRAWL_CODES.LIST_SELECTOR_ZERO_MATCHES))
    || adapterObservedEmpty) {
  ...
  if (codes.size === 0 && !parserObserved && !adapterAuthoritative && !filterObserved) {
    codes.add(OPERATIONAL_CRAWL_CODES.UNKNOWN_NEEDS_MANUAL_REVIEW);
  }
...
function stageStatus(stage, context) {
  ...
  if (stage === "list_fetch") {
    ...
    if (["success", "empty_observed", "partial"].includes(runtimeStatus) || adapterVerified) return "success";
  ...
  if (stage === "notice_url_resolution") {
    ...
    if (metrics.list_candidate_count === 0) return "skipped";
...
function recommendedAction(status, primaryCode) {
  const actions = {
    supported: "none",
...
export function analyzeOperationalCrawlerSource(input = {}) {
  ...
  const operationalCodes = inferOperationalCodes({
    result, parserEvidence, filterMetrics, metrics, profiles, adapterEvidence,
  });
  ...
  return {
    ...
    recommended_action: recommendedAction(status, primaryFailureCode),
