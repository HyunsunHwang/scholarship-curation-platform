...
function classifyOperationalAction({ result, status, codes, remediationEvidence }) {
  ...
  if (comparison && candidate?.control_candidate_count === candidate?.treatment_candidate_count
    && candidate?.removed_candidate_count === 0 && candidate?.added_candidate_count === 0) {
    return { action_class: "no_action", evidence_basis: "same_html_delta" };
  }
  if (comparison && candidate && (candidate?.removed_real_notice_count > 0
    || candidate?.removed_unresolved_count > 0 || candidate?.added_false_positive_count > 0)) {
    return { action_class: "parser_remediation", evidence_basis: "same_html_delta" };
  }
  if (codes.includes(OPERATIONAL_CRAWL_CODES.CONFIG_OR_SELECTOR_MISMATCH)
    || codes.includes(OPERATIONAL_CRAWL_CODES.LIST_SELECTOR_ZERO_MATCHES)) {
    return { action_class: "configuration_review", evidence_basis: "runtime_failure" };
  }
  if (status === "manual_review_required") return { action_class: "unresolved", evidence_basis: "runtime_failure" };
  return { action_class: "no_action", evidence_basis: "runtime_failure" };
}
