...
export function analyzeDetailTextQuality(value) {
  ...
  return {
    ...
    encodingNormalizationApplied: normalizedText !== cleanText(originalText),
    encodingNormalizationImproved: normalizedText !== cleanText(originalText) && replacementCharacterCount === 0,
  };
}
...
export function extractDetailFromCheerio($, options = {}) {
  ...
  if (!content && detailContentSelector) {
    content = cleanText($(detailContentSelector).first().text());
