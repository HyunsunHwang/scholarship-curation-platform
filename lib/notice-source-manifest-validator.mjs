...
function validateUrlTemplate(value, label, allowedPlaceholders, errors) {
  ...
  const rendered = String(value).replace(/\{[^{}]+\}/g, "1");
  ...
  if (source.adapter === "cau_portal") {
    if (!source.adapterConfig) {
      ...
      const unexpected = Object.keys(source.adapterConfig)
        .filter((key) => !["apiAccept", "jsonContentTypePolicy", "requiredListPath"].includes(key));
