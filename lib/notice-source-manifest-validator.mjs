...
import { resolveListParserProfile } from "./crawler-engine/list-parser-profiles.mjs";
...
function validateUrlTemplate(value, label, allowedPlaceholders, errors, { requirePlaceholder = true } = {}) {
  requireTrimmed(value, label, errors, { allowEmpty: false });
  const placeholders = [...String(value ?? "").matchAll(/\{([^{}]+)\}/g)]
    .map((match) => match[1]);
  if (
    (requirePlaceholder && placeholders.length === 0)
    || placeholders.some((placeholder) => !allowedPlaceholders.includes(placeholder))
  ) {
    errors.push(`${label} must use only ${allowedPlaceholders.map((item) => `{${item}}`).join(", ")}`);
    return;
  }
  const rendered = String(value).replace(/\{([^{}]+)\}/g, (_match, placeholder) => (
    placeholder === "origin" ? "https://example.test" : "1"
  ));
  validateHttpUrl(rendered, label, errors, { required: true });
}
...
function validateSource(source, manifestSlug, errors) {
  ...
  if (source.listParserProfile && !resolveListParserProfile(source.listParserProfile)) {
    errors.push(`${source.sourceId}: unknown listParserProfile ${source.listParserProfile}`);
  }
  ...
  if (source.adapter === "cau_portal") {
    if (!source.adapterConfig) {
      ...
      const unexpected = Object.keys(source.adapterConfig)
        .filter((key) => ![
          "apiAccept",
          "jsonContentTypePolicy",
          "requiredListPath",
      ...
      if (unexpected.length > 0) {
        errors.push(`${source.sourceId}: cau_portal adapterConfig has unsupported fields ${unexpected.join(", ")}`);
      }
      validateUrlTemplate(
        source.adapterConfig.requestUrlTemplate,
        `${source.sourceId}.adapterConfig.requestUrlTemplate`,
        [],
        errors,
        { requirePlaceholder: false },
      );
      validateUrlTemplate(
        source.adapterConfig.detailUrlTemplate,
        `${source.sourceId}.adapterConfig.detailUrlTemplate`,
        [],
        errors,
        { requirePlaceholder: false },
      );
      try {
        const listOrigin = new URL(source.listUrl).origin;
        for (const key of ["requestUrlTemplate", "detailUrlTemplate"]) {
          if (new URL(source.adapterConfig[key]).origin !== listOrigin) {
            errors.push(`${source.sourceId}.adapterConfig.${key} must use the list URL origin`);
          }
        }
      } catch {
        // URL validation above records the actionable error.
      }
    }
  } else if (source.adapter === "json_api_board") {
    const config = source.adapterConfig;
    if (!config) {
      errors.push(`${source.sourceId}: json_api_board requires adapterConfig`);
    } else {
      for (const key of [
        "apiAccept",
        "jsonContentTypePolicy",
        ...
        requireTrimmed(config[key], `${source.sourceId}.adapterConfig.${key}`, errors, {
          allowEmpty: false,
        });
      }
      validateUrlTemplate(
        config.requestUrlTemplate,
        `${source.sourceId}.adapterConfig.requestUrlTemplate`,
        ["page"],
        errors,
      );
      ...
      for (const [key, value] of Object.entries(config.fieldMap ?? {})) {
        requireTrimmed(value, `${source.sourceId}.adapterConfig.fieldMap.${key}`, errors, {
          allowEmpty: false,
        });
      }
    }
