...
function isInGateCBaseLineage(repoRoot, commitSha, baseRef) {
  const basePrecedesCommit = isAncestor(repoRoot, baseRef, commitSha);
  if (basePrecedesCommit.environmentError || basePrecedesCommit.value) return basePrecedesCommit;
  // After Gate C is merged, origin/main contains the provenance commit instead
  // of preceding it. Comparable ancestry in either direction preserves the
  // range check without treating an unrelated branch as valid.
  return isAncestor(repoRoot, commitSha, baseRef);
}
...
export function validateGateCProvenance({
  ...
  const corpusIsAfterBase = corpusExists && baseExists
    ? record("corpus_freeze_outside_gate_c_base_lineage", isInGateCBaseLineage(repoRoot, corpusFreezeSha, baseRef))
    : false;
  ...
  const relationIsAfterBase = relationExists && baseExists
    ? record("relation_correction_outside_gate_c_base_lineage", isInGateCBaseLineage(repoRoot, relationCorrectionSha, baseRef))
    : false;
