...
export const P0_REMEDIATED_EXTRACTOR_VERSION = "1.1.1";

const REVIEW_OR_FAILURE_STATUSES = new Set([
...
const SAFE_BODY_STATUSES = new Set([
  "text_sufficient",
  "success",
  "safe",
  "normalized",
...
const SAFE_EXTRACTION_STATUSES = new Set([
  "text_sufficient",
...
const SAFE_QUALITY_STATUSES = new Set([
  "text_sufficient",
  "table_structure_preserved",
  "ocr_succeeded",
  "success",
...
const PHASE3_SUCCESS_STATUSES = new Set([
  "text_sufficient",
  "table_structure_preserved",
  "ocr_succeeded",
  "attachment_primary_content",
...
function buildSegments(sourceNotice, sourceDocuments) {
  ...
  const diagnostics = {
    ...
    attachment_status_rejected_count: 0,
    ocr_missing_locator_count: 0,
    ocr_low_quality_rejected_count: 0,
    ocr_quality_rejected_count: 0,
    phase3_success_status_accepted_count: 0,
  ...
  const bodyUnsafe = sourceNotice.body_manual_review_required === true
    || explicitBodyStatuses.some((status) => REVIEW_OR_FAILURE_STATUSES.has(status) || !SAFE_BODY_STATUSES.has(status))
  ...
  for (const document of allDocuments) {
    ...
    const unsafeStatus = REVIEW_OR_FAILURE_STATUSES.has(document.extraction_status)
      || REVIEW_OR_FAILURE_STATUSES.has(document.quality_status)
      || !SAFE_EXTRACTION_STATUSES.has(document.extraction_status)
      || !SAFE_QUALITY_STATUSES.has(document.quality_status)
    ...
    if (missing.length || unsafeStatus) {
      ...
      if (unsafeStatus) diagnostics.attachment_status_rejected_count += 1;
      if (isOcrDerivedBlock(document) && unsafeStatus) {
        diagnostics.ocr_low_quality_rejected_count += 1;
        diagnostics.ocr_quality_rejected_count += 1;
      }
      reject(`attachment:${document.document_id ?? "unknown"}`, reason, {
        extraction_status: document.extraction_status,
        quality_status: document.quality_status,
      });
    } else if (PHASE3_SUCCESS_STATUSES.has(document.extraction_status) || PHASE3_SUCCESS_STATUSES.has(document.quality_status)) {
      diagnostics.phase3_success_status_accepted_count += 1;
    }
  }
    ...
    if (blocks.length) {
      for (const [blockIndex, block] of blocks.entries()) {
        ...
        const ocrUnsafe = resolvedSourceType === "ocr_text" && (block.page_number == null
          ...
          || (blockStatus && (!SAFE_QUALITY_STATUSES.has(blockStatus) || REVIEW_OR_FAILURE_STATUSES.has(blockStatus)))
        ...
        const segment = makeSegment(sourceNotice, {
          ...
          locator,
          document,
          priority: resolvedSourceType === "ocr_text" ? 2 : block.type === "table" ? 0 : 1,
