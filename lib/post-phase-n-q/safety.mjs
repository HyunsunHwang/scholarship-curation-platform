...
export function extractSupabaseProjectRef(value) {
  ...
  try {
    ...
    const projectUrlMatch = hostname.match(/^([a-z0-9]+)\.supabase\.co$/);
    if (projectUrlMatch) return projectUrlMatch[1];

    const poolerHostMatch = hostname.match(
      /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.pooler\.supabase\.com$/,
    );
    const poolerPortAllowed = url.port === "" || url.port === "5432";
    const postgresProtocol =
      url.protocol === "postgres:" || url.protocol === "postgresql:";
    if (poolerHostMatch && poolerPortAllowed && postgresProtocol) {
      const username = decodeURIComponent(url.username);
      const usernameMatch = username.match(/^postgres\.([a-z0-9]+)$/);
