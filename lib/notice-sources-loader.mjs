...
import { applyListParserProfile } from "./crawler-engine/list-parser-profiles.mjs";
...
function deriveUniversitySlug(sourceId, fallback = "") {
  // Prefer explicit university_slug from CSV/manifest when present.
  // Otherwise fall back to source_id prefix (ewha_001 → ewha).
...
export function mapRawSource(raw) {
  ...
  const adapterConfigRaw = raw.adapter_config ?? raw.adapterConfig;
  const adapterConfig = adapterConfigRaw
    && typeof adapterConfigRaw === "object"
    && !Array.isArray(adapterConfigRaw)
    ? Object.fromEntries(Object.entries(adapterConfigRaw).map(([key, value]) => [
        key,
        typeof value === "string" ? cleanText(value) : value,
      ]))
    : undefined;

  return applyListParserProfile({
    sourceId,
    ...
    detailTitleSelector: cleanText(
      raw.detail_title_selector ?? raw.detailTitleSelector,
    ),
    noticeUrlPattern: cleanText(
      raw.notice_url_pattern ?? raw.noticeUrlPattern,
    ),
    listParserProfile: cleanText(raw.list_parser_profile ?? raw.listParserProfile),
    ...
    undatedCandidatePolicy: cleanText(raw.undated_candidate_policy ?? raw.undatedCandidatePolicy) || "manual_review",
    keywords,
