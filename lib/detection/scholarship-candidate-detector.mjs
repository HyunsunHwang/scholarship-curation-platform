...
export function detectScholarshipCandidate(
  ...
  const allowExplicitly = allowUndated === true
    && !["manual_review", "reject"].includes(undatedCandidatePolicy);
  if (!dateEvidence.rawValue && allowExplicitly) {
  ...
  if (!dateEvidence.rawValue && normalizedStage === "final") {
    base.classification = undatedCandidatePolicy === "reject" ? "not_candidate" : "undetermined";
    base.eligibleForDetailFetch = false;
    base.reasonCodes.push(undatedCandidatePolicy === "reject"
      ? "candidate_date_missing_rejected"
      : "candidate_date_missing_requires_review");
    return base;
  }
  base.reasonCodes.push(
