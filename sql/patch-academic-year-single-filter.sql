-- 학년 매칭 단일화: qual_academic_year만 사용
-- - qual_min_academic_year / qual_min_academic_semester는 더 이상 매칭에 사용하지 않음
-- - 기존 값은 NULL로 정리하고, 이후에도 NULL만 허용

UPDATE public.scholarships
SET
  qual_min_academic_year = NULL,
  qual_min_academic_semester = NULL
WHERE qual_min_academic_year IS NOT NULL
   OR qual_min_academic_semester IS NOT NULL;

ALTER TABLE public.scholarships
  DROP CONSTRAINT IF EXISTS scholarships_qual_min_academic_year_null_only_check,
  DROP CONSTRAINT IF EXISTS scholarships_qual_min_academic_semester_null_only_check;

ALTER TABLE public.scholarships
  ADD CONSTRAINT scholarships_qual_min_academic_year_null_only_check
    CHECK (qual_min_academic_year IS NULL),
  ADD CONSTRAINT scholarships_qual_min_academic_semester_null_only_check
    CHECK (qual_min_academic_semester IS NULL);

COMMENT ON COLUMN public.scholarships.qual_min_academic_year
  IS 'Deprecated: qual_academic_year 단일 조건으로 대체됨. 항상 NULL.';

COMMENT ON COLUMN public.scholarships.qual_min_academic_semester
  IS 'Deprecated: qual_academic_year 단일 조건으로 대체됨. 항상 NULL.';
