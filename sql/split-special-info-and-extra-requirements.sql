-- 특수정보 분리
-- - qual_special_info: 온보딩 special_info enum과 매칭되는 값만 저장
-- - qual_extra_requirements: 상세 표시용 자유 텍스트

ALTER TABLE public.scholarships
  ADD COLUMN IF NOT EXISTS qual_extra_requirements text[];

COMMENT ON COLUMN public.scholarships.qual_extra_requirements IS
  '상세 페이지 표시용 자유 텍스트 요건(매칭 미사용)';

WITH enum_vals AS (
  SELECT enum_range(NULL::public.special_info_type)::text[] AS values
),
mapped AS (
  SELECT
    s.id,
    array_agg(req) FILTER (WHERE req = ANY(e.values)) AS matched_values,
    array_agg(req) FILTER (WHERE NOT (req = ANY(e.values))) AS extra_values
  FROM public.scholarships s
  CROSS JOIN enum_vals e
  LEFT JOIN LATERAL unnest(coalesce(s.qual_special_info, '{}'::text[])) AS req ON true
  GROUP BY s.id
)
UPDATE public.scholarships s
SET
  qual_special_info = nullif(
    ARRAY(
      SELECT DISTINCT value
      FROM unnest(coalesce(mapped.matched_values, '{}'::text[])) AS value
      WHERE btrim(value) <> ''
    ),
    '{}'::text[]
  ),
  qual_extra_requirements = nullif(
    ARRAY(
      SELECT DISTINCT value
      FROM unnest(
        coalesce(s.qual_extra_requirements, '{}'::text[])
        || coalesce(mapped.extra_values, '{}'::text[])
      ) AS value
      WHERE btrim(value) <> ''
    ),
    '{}'::text[]
...
ALTER TABLE public.scholarships
  DROP CONSTRAINT IF EXISTS scholarships_qual_special_info_enum_only_check;

ALTER TABLE public.scholarships
  ADD CONSTRAINT scholarships_qual_special_info_enum_only_check
  CHECK (
    qual_special_info IS NULL
    OR qual_special_info <@ enum_range(NULL::public.special_info_type)::text[]
  );
