-- 장학금 자격요건: 입학 구분 제한(일반입학/편입학/재입학) 추가
ALTER TABLE public.scholarships
  ADD COLUMN IF NOT EXISTS qual_admission_type text[];

ALTER TABLE public.scholarships
  DROP CONSTRAINT IF EXISTS scholarships_qual_admission_type_check;
ALTER TABLE public.scholarships
  ADD CONSTRAINT scholarships_qual_admission_type_check
  CHECK (
    qual_admission_type IS NULL
    OR qual_admission_type <@ ARRAY['일반입학', '편입학', '재입학']::text[]
  );

COMMENT ON COLUMN public.scholarships.qual_admission_type
  IS '입학 구분 제한(일반입학/편입학/재입학), NULL/빈배열이면 제한 없음';

-- 입학 구분 필터를 포함하도록 매칭 함수 갱신
