...
- `CRAWL_TIMEOUT_MS` and `CRAWL_RETRY_COUNT` configure the authoritative CLI path.
...
The aggregate includes requested/completed/success/failed/timeout/zero-match/partial/blocked counts; total attempts and items; retry/recovery/exhaustion counts; per-source final status; and `overall_run_status`.
...
`validateCrawlerRunSummary` checks source count, mutually exclusive status arithmetic, attempt totals, item totals, and retry count bounds.
...
The committed tests cover healthy execution, transient recovery, timeout recovery and exhaustion, non-retryable failures, HTTP retry policy, zero-match, multi-source isolation, partial/all-failed runs, item deduplication, per-source bounds, secret redaction, normalized graph compatibility, detail partials, timer/request cleanup, retry aggregate counts, and run-summary arithmetic.

## Bounded live observation

The live dry-run used `ewha_010` and `ewha_015`, sequential execution, one list page, at most three observed items per source, 25-second timeout, and one retry. Both sources completed in one attempt with three observed items each, no timeout, no retry, no source error, and no detail error.
