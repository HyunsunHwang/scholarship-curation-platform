...
The evaluator reuses Gate A metric names. Classification, annotated field presence, type-aware normalized exact match, evidence attribution, unsupported value rate, and risky-case review recall are measured. Exact match and partial match are separate metrics: the current fixtures have two exact normalized-value gold annotations but no predeclared set/range/condition partial-overlap gold annotations. Therefore `normalized_exact_match` is reported with `sample_count=2`, while `normalized_partial_match` is explicitly `not_evaluated` with `sample_count=0` and a concrete reason. It is never copied from the exact score. Identity pair precision/recall and material-change classification are also explicitly `not_evaluated` because Gate B has neither a pair resolver nor before/after comparator.

An empty denominator is not 100%. Every ratio with no evaluation sample is represented as `status=not_evaluated`, `value=null`, `sample_count=0`, and a non-empty reason. A `not_evaluated` diagnostic metric is not automatically treated as an acceptance PASS. Gate B acceptance continues to depend only on the contract-required schema, evidence, unsupported-value, deterministic-rerun, publication/notification, risky-review, identity-evidence, and no-invented-change conditions.

Evaluation results are sliced by field, evidence source type, document kind, value status, extractor kind, and fixture version. Each evaluated slice carries a positive sample count and applicable counts/ratios. Empty coverage such as model extraction, HWP evidence attribution, or fixture-absent document kinds remains `not_evaluated`; it is not assigned a synthetic score. The source-type slice distinguishes the supported capability matrix from patterns actually covered by gold annotations.

All reported 100% values describe only the selected annotations in these 15 synthetic contract fixtures. They are not production accuracy, representative-source accuracy, or a production threshold. A representative public gold evaluation with adjudicated field values, evidence attribution, source/document diversity, and genuine partial-overlap annotations remains required in later Phase 4 work. Whether limited Phase 4C model integration is needed before a Phase 5 prototype must be decided from that representative evaluation, not from the synthetic fixture scores alone.
