...
import { buildPhase2CandidateComparison, buildPhase3RemediationClusters, classifyCandidateFingerprint, normalizeCandidateLocator } from "../lib/crawler-engine/runtime-diagnostics/index.mjs";
...
function evidence(sourceId, { origin = "https://one.test", family = "table", recall = true, adapter = null } = {}) {
  ...
  const treatment = { ...control, candidates: treatmentCandidates, parser_config: { list_item_selector: family === "table" ? "table.board tr" : "ul.cards li", link_selector: "a[href]", title_selector: ".subject", date_selector: ".date", list_parser_profile: null, adapter }, parser_evidence: { parser_strategy: "configured_selector", configured_list_selector: family === "table" ? "table.board tr" : "ul.cards li", matched_list_selector: family === "table" ? "table.board tr" : "ul.cards li", profile_applied: false, inline_notice_section_count: 0, independent_detail_url_count: 1 } };
...
assert.deepEqual(classifyCandidateFingerprint("configured_selector:https://a.test/board/view?id=123"), { kind: "url_based", candidate_origin: "configured_selector", dom_locator: null });
assert.equal(normalizeCandidateLocator({ provenance: { candidate_node_fingerprint: "configured_selector:table.board tr:nth-child(12)" } }), "table.board tr:nth-child(*)");
assert.equal(normalizeCandidateLocator({ provenance: { dom_locator: "ul.cards li:nth-of-type(3)", candidate_node_fingerprint: "configured_selector:https://a.test/board/view?id=123" } }), "ul.cards li:nth-of-type(*)");
const adapterClusters = buildPhase3RemediationClusters({ baseline: { target_count: 2, targets: [{ source_id: "adapter_a" }, { source_id: "adapter_b" }] }, sameCaptureEvidence: [evidence("adapter_a", { adapter: "generic_board" }), evidence("adapter_b", { adapter: "form_json_board" })] });
assert.equal(adapterClusters.clusters.length, 2);
...
invalid((value) => { value.capture_id = ""; }, /capture ID/);
invalid((value) => { value.capture.requested_url = ""; }, /requested_url provenance/);
invalid((value) => { value.capture.final_url = "ftp://one.test/board"; value.control.final_url = "ftp://one.test/board"; value.treatment.final_url = "ftp://one.test/board"; }, /final_url provenance/);
invalid((value) => { value.control.source_id = ""; }, /source provenance/);
...
console.log("phase3_parser_remediation_cluster_tests_passed=20");
