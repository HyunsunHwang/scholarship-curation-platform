...
const inputPath =
  process.argv[2] ??
  String.raw`C:\Users\user\Downloads\시립대_보완.csv`;
const outputPath = process.argv[3] ?? "data/notice-sources-uos.csv";
...
const requiredColumns = ["college_name", "department_name", "공지사항 링크"];
...
for (const source of body) {
  ...
  rows.push({
    source_id: `uos_${String(rows.length + 1).padStart(3, "0")}`,
    source_name: `시립대 ${department}`,
    list_url: listUrl,
    base_url: origin,
    list_item_selector: "",
    link_selector: "",
