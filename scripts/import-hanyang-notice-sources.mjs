...
const inputPath =
  process.argv[2] ??
  String.raw`C:\Users\user\Downloads\한양대_보완.csv`;
const outputPath = process.argv[3] ?? "data/notice-sources-hanyang.csv";
...
for (const source of body) {
  ...
  rows.push({
    source_id: `hanyang_${String(rows.length + 1).padStart(3, "0")}`,
    university_slug: "hanyang",
    college_name: college,
    department_name: department,
    source_level: "department",
    source_name: `한양대 ${department}`,
    list_url: listUrl,
    base_url: origin,
    list_item_selector: "",
    link_selector: "",
