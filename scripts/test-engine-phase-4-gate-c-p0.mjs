...
const productionValidation = validateProductionSourceReview(corpus, productionSourceReview);
assert.equal(productionValidation.valid, true, productionValidation.errors.join("; "));
assert.equal(productionSourceReview.cases.length, 19);
assert.equal(productionSourceReview.cases.every((item) => item.review_status === "reviewed" && !Object.hasOwn(item.fields, "timezone")), true);
assert.equal(productionSourceReview.cases.filter((item) => item.terminal_non_opportunity).every((item) => Object.values(item.fields).every((field) => field.status === "not_applicable")), true);
console.log("PASS Batch 2 production-source review is complete, bounded, and terminal-safe");

const invalidProductionReview = JSON.parse(JSON.stringify(productionSourceReview));
invalidProductionReview.cases[0].fields.timezone = { status: "present", normalized_value: "Asia/Seoul", support_basis: ["production_source_reviewed"], note: null };
assert.equal(validateProductionSourceReview(corpus, invalidProductionReview).valid, false);
const invalidUnresolvedBasis = JSON.parse(JSON.stringify(productionSourceReview));
const unresolvedField = Object.values(invalidUnresolvedBasis.cases.find((item) => Object.values(item.fields).some((field) => field.status === "unresolved")).fields).find((field) => field.status === "unresolved");
unresolvedField.support_basis = ["production_source_reviewed"];
assert.equal(validateProductionSourceReview(corpus, invalidUnresolvedBasis).valid, false);
console.log("PASS production-source review rejects standalone timezone and unsupported unresolved decisions");

const records = buildRecords();
const first = evaluateP0Audit({ corpus, adjudicationDecisions: decisions, overlay, recordsByCase: records, productionSourceReview });
const second = evaluateP0Audit({ corpus, adjudicationDecisions: decisions, overlay, recordsByCase: buildRecords(), productionSourceReview });
...
assert.equal(first.p0_contract.standalone_timezone_field, false);
assert.equal(first.p0_contract.opportunity_fields.includes("timezone"), false);
assert.equal(first.production_source_review.combined_p0_case_review_count, 24);
assert.deepEqual(first.production_source_review.date_normalization, { reviewed_field_count: 38, applicable_field_count: 32, present_count: 30, date_only_count: 15, offset_datetime_count: 15, unresolved_or_not_found_count: 2, not_applicable_count: 6 });
assert.equal(first.production_source_review.amount_semantics.semantically_resolved_count, 15);
assert.equal(first.production_source_review.amount_semantics.canonical_schema_representable_count, 6);
assert.equal(first.production_source_review.amount_semantics.schema_gap_count, 9);
console.log("PASS repeated audit is byte-equivalent with fixed as_of");

assert.equal(first.corpus.total_p0_field_count, 216);
assert.equal(first.corpus.resolved_p0_field_count, 14);
assert.equal(first.corpus.pending_p0_field_count, 198);
...
const oneResolved = evaluateP0Audit({ corpus, adjudicationDecisions: oneApproved, overlay, recordsByCase: records, productionSourceReview });
assert.equal(oneResolved.corpus.resolved_p0_field_count, 15);
...
const automatedResult = evaluateP0Audit({ corpus, adjudicationDecisions: automatedApproval, overlay, recordsByCase: records, productionSourceReview });
assert.equal(automatedResult.corpus.resolved_p0_field_count, 14);
...
const oneSafetyResolved = evaluateP0Audit({ corpus, adjudicationDecisions: oneSafetyApproved, overlay, recordsByCase: records, productionSourceReview });
...
console.log("PASS Case 4 publishability and Case 5 embedded-offset date decisions are evaluated explicitly");
...
const unsupported = evaluateP0Audit({ corpus, adjudicationDecisions: decisions, overlay, recordsByCase: unsupportedRecords, productionSourceReview });
