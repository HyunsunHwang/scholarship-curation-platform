...
const inputPath = process.argv[2] ?? "data/notice-sources-korea.csv";
...
const outputRows = body.map((row) => {
  const next = [...row];
  const sourceId = cleanText(next[index.source_id]);
  const listUrl = cleanText(next[index.list_url]).toLowerCase();
  if (!sourceId.startsWith("korea_")) return next;
  ...
  if (isDoBoard) {
    ...
    // html 기반 게시판은 구조가 다양해서 셀렉터는 비워두고 키워드만 적용
    if (!cleanText(next[index.link_selector])) {
      next[index.link_selector] = "a[href]";
      tunedCount += 1;
    }
