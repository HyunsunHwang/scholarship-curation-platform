...
function main() {
  ...
  const tests = [
    ...
    { name: "F-0, B/C, E, A, F-2, and F-3 reports are all referenced", pass: Object.keys(first.input_reports).length === 9 && first.review_quality_summary.no_assets_needs_review_count === inputs.bc.counts.no_assets_needs_review_count && first.batch_summary.batch_status === inputs.e.batch_status && first.f2_summary.p0_deferred_count === inputs.f2.metrics.p0_deferred_count && first.f3_summary.p1_deferred_count === inputs.f3.metrics.p1_deferred_count && page.includes("getAdminCrawlerReviewDiagnostics") && helper.includes("post-phase-f1-admin-review-integration") },
    ...
    { name: "F-2 bounded remediation notices and statuses are visible", pass: first.scope_notices.some((note) => note.includes("F-2 resolves only bounded")) && first.scope_notices.some((note) => note.includes("Fail-closed")) && first.diagnostics.some((item) => item.f2RemediationStatus === "resolved") && first.diagnostics.some((item) => item.f2RemediationStatus === "deferred") },
    { name: "F-3 bounded remediation notices and statuses are visible", pass: first.scope_notices.some((note) => note.includes("F-3 resolves only bounded")) && first.scope_notices.some((note) => note.includes("Attachment-only or mojibake")) && first.diagnostics.some((item) => item.f3RemediationStatus === "resolved") && first.diagnostics.some((item) => item.f3RemediationStatus === "deferred") },
  ...
  const validation = { generated_at: first.generated_at, status: tests.every((test) => test.pass) ? "PASS" : "HOLD", metrics: { ...first.metrics, deterministic_rerun_match: tests[7].pass, output_schema_valid: tests.slice(0, 7).every((test) => test.pass), safety_valid: tests[8].pass, restricted_file_scope_valid: tests[9].pass }, tests, safety: { db_access: false, db_write: false, supabase_access: false, migration: false, production_crawler_execution: false, production_detector_parser_change: false, destructive_action: false, admin_write_path_added: false } };
