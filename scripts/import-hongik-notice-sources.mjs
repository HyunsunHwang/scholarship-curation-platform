...
const inputPath =
  process.argv[2] ??
  String.raw`C:\Users\user\Downloads\홍익대학교.csv`;
const outputPath = process.argv[3] ?? "data/notice-sources-hongik.csv";
...
const requiredColumns = ["university_name", "college_name", "department_name", "공지사항 링크"];
...
for (const source of body) {
  const college = cleanText(source[index.college_name]);
  const department = cleanText(source[index.department_name]);
  ...
  rows.push({
    source_id: `hongik_${String(rows.length + 1).padStart(3, "0")}`,
    university_slug: "hongik",
    college_name: college,
    department_name: department,
    source_level: "department",
    source_name: `홍익대 ${department}`,
    list_url: listUrl,
    base_url: origin,
    list_item_selector: "",
    link_selector: "",
