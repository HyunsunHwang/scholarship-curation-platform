...
const inputPath =
  process.argv[2] ??
  String.raw`C:\Users\user\Downloads\중앙대_보완.csv`;
const outputPath = process.argv[3] ?? "data/notice-sources-cau.csv";
...
for (const source of body) {
  ...
  rows.push({
    source_id: `cau_${String(rows.length + 1).padStart(3, "0")}`,
    source_name: `중앙대 ${department}`,
    list_url: listUrl,
    base_url: origin,
    list_item_selector: "",
    link_selector: "",
