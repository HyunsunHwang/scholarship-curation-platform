...
const inputPath =
  process.argv[2] ??
  String.raw`C:\Users\user\Downloads\경희대_보완.csv`;
const outputPath = process.argv[3] ?? "data/notice-sources-khu.csv";
...
for (const source of body) {
  ...
  rows.push({
    source_id: `khu_${String(rows.length + 1).padStart(3, "0")}`,
    university_slug: "khu",
    college_name: college,
    department_name: department,
    source_level: "department",
    source_name: `경희대 ${department}`,
    list_url: listUrl,
    base_url: origin,
    list_item_selector: "",
    link_selector: "",
