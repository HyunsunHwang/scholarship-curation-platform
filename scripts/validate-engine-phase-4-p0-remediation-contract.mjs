...
check("unknown lifecycle accepts date, classification, relation, and publishability causes", ["classification_uncertain", "relation_resolution_required", "publishability_requires_confirmation"].every((reason) => FIELD_REVIEW_POLICY.field_reason_options.lifecycle_status.includes(reason)));
...
check("classification and relation fixtures keep clear dates without false missing-window reasons", [byScenario.unknown_document_requires_review, byScenario.standalone_correction_notice].every((item) => item.fields.application_start.status === "present" && item.fields.application_deadline.status === "present" && item.fields.lifecycle_status.status === "unknown" && !item.review.reasons.includes("missing_primary_application_window")));
...
check("non-publishable recruitment policy requires confirmation review", report.recruitment_suppression_policy.when_publishable_opportunity_false.review_required && report.recruitment_suppression_policy.when_publishable_opportunity_false.required_reason === "publishability_requires_confirmation" && report.recruitment_suppression_policy.automatic_publish_allowed === false);
...
check("URL route identity policy ignores protocol and normalizes ports", report.url_route_identity_policy.protocol_ignored && report.url_route_identity_policy.default_ports_normalized && report.url_route_identity_policy.non_default_port_significant && report.url_route_identity_policy.query_ignored && report.url_route_identity_policy.fragment_ignored && report.url_route_identity_policy.trailing_slash_ignored);
