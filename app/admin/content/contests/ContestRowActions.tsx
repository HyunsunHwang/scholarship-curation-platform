...
import { deleteContest, toggleContestVerified, toggleContestRecommended } from "./actions";
...
export function ToggleVerifiedButton({
  ...
  return (
    <button
      onClick={() => startTransition(() => void toggleContestVerified(id, isVerified))}
      disabled={isPending}
...
export function ToggleRecommendedButton({
  ...
  return (
    <button
      type="button"
      onClick={() => startTransition(() => void toggleContestRecommended(id, isRecommended))}
      disabled={isPending}
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition-colors disabled:opacity-50 ${
        isRecommended
...
export function DeleteButton({ id }: { id: number }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(() => void deleteContest(id));
  };
