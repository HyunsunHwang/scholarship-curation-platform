...
export default async function ContestDetailPage({
  ...
  const [{ data: contest }, { data: { user } }, { data: selectionStages }] =
    await Promise.all([
      publicClient.from("contests").select("*").eq("id", contestId).eq("is_verified", true).maybeSingle(),
  ...
  const organizationInitial = contest.organization?.charAt(0) || "공";
            ...
            <div className="grid w-full grid-cols-1 items-start gap-0 md:grid-cols-[1fr_260px] md:gap-10">
              <div className="w-full min-w-0 overflow-x-hidden md:order-1">
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 border-t border-gray-100 pt-6">
                  <div>
                    <p className="text-xs text-ink/50">시상 규모</p>
                    <p
